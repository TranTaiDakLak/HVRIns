...
| `result` | 5 | ⚠️ **Package tái tạo** — xem ghi chú bên dưới | output/repository |

> ⚠️ **`internal/result` — package tái tạo (D-012, 2026-06-21):** Package này bị import bởi
> `app.go`, `app_register.go`, `app_verify.go` nhưng **không có trong repo gốc** (repo không build
> được). Dev 1 đã dựng lại bằng suy luận từ usage patterns (commit a3d8210) để unblock build.
> Hệ quả: `writer.go`/`counter.go` implement đúng; nhưng `format.go` (thứ tự field FormatReg/
> FormatVerify), `files.go` (tên file constant), và `dispatch.go` (logic ghi detail-file) là **suy
> luận có thể lệch bản gốc**. Phải validate hành vi output thật trước khi dùng production.
