...
| D-012 | 2026-06-21 | **`internal/result`: tìm thấy BẢN GỐC ở repo tổ tiên `D:\Github\HVR\HVR` → KHÔI PHỤC verbatim, bỏ bản tái tạo** | Bản tái tạo (a3d8210) build xanh nhưng SAI hành vi: 3 filename constant sai (`DieAfterVerify.txt`→đúng `Die.txt`; `UnknownError_CheckLiveDie.txt`→đúng `Unknown.txt`; `UnknownBlock.txt`→đúng `UnknownReg.txt`), `dispatch.go` là STUB trả nil (mất 15+ detail-file phân loại lỗi), thiếu `counters.go`/`errorlog.go`/`store.go`/test. Bản gốc tự-chứa (chỉ stdlib, không cần đổi module path), call site giống HVR **byte-for-byte** (drift lens). Validate: `go build .`/`go test ./internal/result/...` (TestFormatReg/FormatVerify + UpsertUID)/`go vet`/`wails build` đều PASS; consumer `autoDetectAccount` parse theo pattern (không theo vị trí) nên khớp; `app_register.go:4536` & `verify/web/verify.go:616-618` hardcode `Unknown.txt`/`Die.txt` khớp đúng bản gốc. 0 critical. | Chốt (S05-D1-T001) |
