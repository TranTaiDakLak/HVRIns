// body.go — Form data builder cho iOS HTTP (MFB) register POST
...
// NOTE: iOS HTTP và WebAndroid dùng cùng body format (C# CreateAccountChromeAndroidFormData)
package ioshttp
...
// postURL tạo URL POST endpoint với versioningID (New UI)
...
// oldUIPostURL tạo URL POST /reg/submit/ cho Old UI
// C#: urlSubmitReg = "https://m.facebook.com/reg/submit/" + "?privacy_mutation_token=" + regPrivacyTokenPath
func oldUIPostURL(privacyToken string) string {
	base := "https://m.facebook.com/reg/submit/"
	if privacyToken != "" {
		return base + "?privacy_mutation_token=" + privacyToken
	}
	return base
}

// buildOldUIRegisterBody tạo form-urlencoded body cho Old UI register POST
// Mapping từ C#: FacebookApiFormDataBuilder.RegisterMfbOldUIFormData()
func buildOldUIRegisterBody(p regParams, tokens pageTokens) string {
	ts := fmt.Sprintf("%d", time.Now().Unix())

	// Parse birthday "DD-MM-YYYY" → day, month, year
	day, month, year := parseBirthday(p.birthday)

	// encpass — C# hardcodes %23 cho '#' và %3A cho ':', password gửi raw
	// C#: encpass=%23PWD_BROWSER%3A0%3A{ts}%3A{password}
	encpass := fmt.Sprintf("%%23PWD_BROWSER%%3A0%%3A%s%%3A%s", ts, p.password)
	...
	body := fmt.Sprintf(
		"ccp=2"+
			...
			"&app_id="+
			"&logger_id=%s"+
			"&field_names%%5B0%%5D=firstname"+
			"&firstname=%s"+
			"&lastname=%s"+
			"&field_names%%5B1%%5D=birthday_wrapper"+
			"&birthday_month=%d"+
			"&birthday_day=%d"+
			"&birthday_year=%d"+
			"&age_step_input="+
			"&did_use_age=false"+
			"&field_names%%5B2%%5D=reg_email__"+
			"&reg_email__=%s"+
			"&field_names%%5B3%%5D=sex"+
			"&sex=%d"+
			"&preferred_pronoun="+
			"&custom_gender="+
			"&field_names%%5B4%%5D=reg_passwd__"+
			...
			"&jazoest=%s"+
			"&lsd=%s"+
			"&__dyn=&__csr=&__hsdp=&__hblp=&__sjsp=&__req=h&__fmt=1"+
			"&__a=%s"+
			"&__user=0",
		tokens.regInstance,
		tokens.regImpressionID,
		tokens.loggerID,
		p.firstName,
		p.lastName,
		month, day, year,
...
// parseBirthday tách "DD-MM-YYYY" thành (day, month, year)
func parseBirthday(birthday string) (day, month, year int) {
