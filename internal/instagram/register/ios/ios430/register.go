...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformIOS430, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func doRegisterAccount(ctx context.Context, input *instagram.RegInput, onStatus func(string), pickedDatrOut *string) *instagram.RegResult {
	...
	if SharedDatrPool != nil {
		if datr := SharedDatrPool.GetNext(slotIdx); datr != "" {
			...
			notify(fmt.Sprintf("[iOS430] Dùng datr %s... | used=%d S/F/U=%d/%d/%d",
				pfx, used, s, f, u))
		} else {
			notify("[iOS430] ⚠️ DatrPool EMPTY — dùng MachineID random")
		}
	}
	...
	notify(fmt.Sprintf("[iOS430] POST create.account (%s)...", profile.Device.FBDV))
	...
	notify(fmt.Sprintf("[iOS430] Response (%d bytes)", len(respBody)))
	...
	for round := 2; round <= maxRounds; round++ {
		if !(outcome.UID != "" && outcome.AccessToken == "" && outcome.Partial != nil) {
			if outcome.UID != "" && outcome.AccessToken == "" {
				notify(fmt.Sprintf("[iOS430] Round %d skip — Partial nil", round))
			}
			break
		}
		notify(fmt.Sprintf("[iOS430] Round %d (partial=%d, srn=%d, regctx=%d)",
		...
		if berr != nil {
			notify(fmt.Sprintf("[iOS430] Round %d build body lỗi: %v", round, berr))
			break
		}
		...
		notify(fmt.Sprintf("[iOS430] Round %d resp %d bytes, err=%v", round, len(respN), herr))
		if perrN != nil || outN == nil || outN.UID == "" {
			break
		}
		// Round N trả full → done.
		if outN.AccessToken != "" || outN.Cookie != "" {
			notify(fmt.Sprintf("[iOS430] Round %d OK — token=%v cookie=%v",
