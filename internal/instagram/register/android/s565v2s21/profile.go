// profile.go â€” S565 device profile + UA builder + cookie seed + shared pool.
package s565v2s21
...
// S565S21Profile holds all S565-specific data for one registration session.
type S565S21Profile struct {
	fakeinfo.FullRegProfile           // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	S565UA                  string    // S565-specific User-Agent (FBAV 563)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S565 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S565S21Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S565S21Profile {
	...
	return S565S21Profile{
		FullRegProfile: base,
		Device:         device,
		S565UA:         uaRes.UserAgent,
		ConnType:       connType,
	}
}
