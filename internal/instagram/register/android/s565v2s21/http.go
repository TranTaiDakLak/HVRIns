...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s565v2s21 tls client: %w", err)
	}
...
func buildHeaders(profile S565S21Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.S565UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s565FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s565OAuthToken},
		[2]string{"x-meta-zca", s565MetaZCA},
