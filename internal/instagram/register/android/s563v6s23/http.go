// http.go — S563V4S21 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create S563V4S21 tls client: %w", err)
	}
...
// buildHeaders — S563V4S21 header format (port từ s562v3, build .26).
