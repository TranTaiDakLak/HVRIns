...
// Platform "s565s23": FBAV/565.0.0.0.28, new bloks_versioning_id/doc_id, gzip body,
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS565S23, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s565s23", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s565s23", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s565s23] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s565s23] âš ï¸ Pool EMPTY (slot=%d) â€” reg KHÃ”NG cÃ³ datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s565s23] DÃ¹ng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify("[s565s23] Báº¯t Ä‘áº§u reg")
	...
	notify(fmt.Sprintf("[s565s23] Start â€” %s %s | %s | %s | seed=%s",
	...
	notify("[s565s23] Password encrypted")
	...
	notify(fmt.Sprintf("[s565s23] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s565s23] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s565s23] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s565s23] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s565s23] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
