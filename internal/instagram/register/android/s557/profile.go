// profile.go — S557 device profile + UA builder + cookie seed + shared pool.
package s557
...
// S557Profile holds all S557-specific data for one registration session.
type S557Profile struct {
	fakeinfo.FullRegProfile          // embed base profile
	Device    S23Device              // device variant (S23Device defined in body.go)
	S557UA    string                 // S557-specific User-Agent (FBAV 557)
...
// BuildProfileForPlatform builds a complete S557 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S557Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S557Profile {
