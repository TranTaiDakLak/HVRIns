// Package s557 — Samsung Galaxy S23 + Facebook API 557 registration.
// Platform "s557": FBAV/557.0.0.59.72, new bloks_versioning_id, gzip body,
// updated headers (appnetsession, tasos, priority, qpl-active-flows),
// removed X-Fb-Privacy-Context / x-zero-state / x-zero-eh / X-Fb-Rmd.
//
// KHÔNG đụng đến register/s23 — đây là platform riêng biệt.
package s557
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS557, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
type WorkerContext struct {
	sess        *session
	profile     S557Profile
	platform    string // cache cho SetUAOptions rebuild
	countryCode string // cache cho SetUAOptions rebuild
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s557", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s557", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Profile() S557Profile { return w.profile }
func (w *WorkerContext) UserAgent() string     { return w.profile.S557UA }
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if input != nil {
		...
		if input.UserAgent != "" {
			...
			profile.S557UA = input.UserAgent
		}
		// UseOriginalUA: force device/locale/SIM khớp OriginalUA (SM-S911B, en_GB, samsung
		// + FBCR carrier theo IP). Tránh fingerprint mismatch UA vs body/headers.
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s557] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s557] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s557] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s557] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s557] Password encrypted")
	...
	notify(fmt.Sprintf("[s557] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s557] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s557] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s557] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s557] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S557 %s)", parsed.UID, profile.Device.Model),
		UserAgent:      profile.S557UA,
