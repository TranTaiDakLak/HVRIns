// http.go — S557 HTTP transport + request headers.
// Dùng cùng format API/headers/body với s23 — chỉ khác FB app version trong UA.
package s557
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s557 tls client: %w", err)
	}
	return &session{client: c}, nil
}

// postGzip gzip-compresses body và gửi POST — 557 traffic gửi gzip (content-encoding: gzip).
...
// buildHeaders — 557 header format (captured traffic).
func buildHeaders(profile S557Profile) [][2]string {
	...
	h := [][2]string{
		...
		{"user-agent", profile.S557UA},
		{"x-graphql-request-purpose", "fetch"},
		{"x-fb-friendly-name", s557FriendlyName},
...
// connUUID — x-fb-conn-uuid-client: base64(16 raw UUID bytes) — 557 format.
