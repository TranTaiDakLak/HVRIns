// http.go — s538 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s538 tls client: %w", err)
	}
...
// buildHeaders — s538 header format (captured traffic — identical với S559).
func buildHeaders(profile s538Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s538UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s538FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s538OAuthToken},
		[2]string{"x-meta-zca", s538MetaZCA},
