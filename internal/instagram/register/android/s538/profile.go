// profile.go — s538 device profile + UA builder + cookie seed + shared pool.
package s538
...
// s538Profile holds all s538-specific data for one registration session.
type s538Profile struct {
...
// BuildProfileForPlatform builds a complete s538 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s538Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s538Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s538 luôn ra
	// FBAV/538.0.0.53.70 bất kể có file Config riêng hay không.
