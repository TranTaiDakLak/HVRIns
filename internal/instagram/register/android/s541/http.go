// http.go — s541 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s541 tls client: %w", err)
	}
...
// buildHeaders — s541 header format (captured traffic — identical với S559).
func buildHeaders(profile s541Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s541UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s541FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s541OAuthToken},
		[2]string{"x-meta-zca", s541MetaZCA},
