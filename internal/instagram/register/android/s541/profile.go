// profile.go — s541 device profile + UA builder + cookie seed + shared pool.
package s541
...
// s541Profile holds all s541-specific data for one registration session.
type s541Profile struct {
...
// BuildProfileForPlatform builds a complete s541 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s541Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s541Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s541 luôn ra
	// FBAV/541.0.0.85.79 bất kể có file Config riêng hay không.
