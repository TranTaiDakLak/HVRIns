// profile.go — S561V4S21 device profile + UA builder + cookie seed + shared pool.
...
// S560Profile holds all S561V4S21-specific data for one registration session.
type S560Profile struct {
	...
	S560UA                  string    // S561V4S21-specific User-Agent (FBAV 561, SM-G996B)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S561V4S21 registration profile (default UA).
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S560Profile {
	...
	ua := buildS561V4S21UA(device, locale, carrier, addVirtualSpecs, r)
...
// buildS561V4S21UA — FB4A native UA cho SM-G996B (Galaxy S21+), version đọc từ versions_and_builds.txt.
func buildS561V4S21UA(device S23Device, locale, carrier string, addVirtualSpecs bool, r *rand.Rand) string {
