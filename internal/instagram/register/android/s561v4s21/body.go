// body.go — S561V4S21 register POST body builder + app constants.
// FBAV/561.0.0.42.67, FBBV/976056141 — captured 2026-05 từ thiết bị Samsung Galaxy S21+ (SM-G996B).
// Cùng doc_id/bloks_ver như s561v3 nhưng FBBV mới + theme XMDS+FDS + device S21+.
// Khác s563s21: FBAV 561 (không phải 563), doc_id v561, theme XMDS+FDS (không phải FDS-only).
package s561v4s21
...
// ─── S561V4S21 app constants ─────────────────────────────────────────────────────

var S560AppVersions = []struct {
	Version string // FBAV
	Build   string // FBBV
}{
	{"561.0.0.42.67", "976056141"}, // captured 561v2 mới (S21+, 2026-05) — confirmed
}

const (
	S560DocID        = "119940804217607550791392237684"
	S560BloksVer     = "9d448b7c3b47250635f0acbdd801409700933b0eba77ec236358984692f4d562"
	S560OAuthToken   = "350685531728|62f8ce9f74b12f84c123cc23437a4a32"
	S560MetaZCA      = "empty_token"
	S560FriendlyName = "FbBloksActionRootQuery-com.bloks.www.bloks.caa.reg.create.account.async"
	S560StylesID     = "8a1cdd5e66badb0c4d074cbdf05d3791"
	S560ZeroEH       = "664c0faaac849cb891d0a261fbb72a12"

	// OriginalUA — UA gốc cố định cho platform S561V4S21 (SM-G996B Galaxy S21+, en_GB, Viettel).
	OriginalUA = "[FBAN/FB4A;FBAV/561.0.0.42.67;FBBV/976056141;FBDM/{density=2.8125,width=1080,height=2400};FBLC/en_GB;FBRV/0;FBCR/Viettel;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-G996B;FBSV/15;FBOP/1;FBCA/arm64-v8a:;]"
)

// S23Device represents a Samsung device variant (reused for S561V4S21 — Galaxy S21 family).
...
// s21Devices — Galaxy S21 family (SM-G99xB). Density 2.8125, 1080x2400 (FBSS=3).
var s21Devices = []S23Device{
	{Model: "SM-G996B", Name: "Galaxy S21+ 5G", Width: 1080, Height: 2400, Density: "2.8125", FBSS: "3"},
	{Model: "SM-G991B", Name: "Galaxy S21 5G", Width: 1080, Height: 2400, Density: "2.8125", FBSS: "3"},
	{Model: "SM-G996U", Name: "Galaxy S21+ US", Width: 1080, Height: 2400, Density: "2.8125", FBSS: "3"},
}
...
func S560RandomBase64URLRaw(n int) string {
	b := make([]byte, n)
...
func S560AACJSON(ts int64) string {
	return fmt.Sprintf(
		`{"aac_init_timestamp":%d,"aaccs":"%s","aacjid":"%s"}`,
		ts, S560RandomBase64URLRaw(32), uuid.New().String(),
	)
}

func S560EscL3StringValue(v string) string {
	escaped := url.QueryEscape(v)
	escaped = strings.ReplaceAll(escaped, "%22", "%5C%5C%5C%5C%5C%5C%5C%22")
	return strings.ReplaceAll(escaped, "+", "%20")
}

func S560FakeSafetyNetToken(ts int64) string {
	return S560RandomBase64URLRaw(18) + "." + fmt.Sprintf("%d", ts) + "." + S560RandomBase64URLRaw(48)
}

func S560FakePlayIntegrityToken() string {
	return strings.Join([]string{
		"eyJhbGciOiJSU0EtT0FFUCIsImVuYyI6IkExMjhHQ00ifQ",
		S560RandomBase64URLRaw(64),
		S560RandomBase64URLRaw(16),
		S560RandomBase64URLRaw(96),
		S560RandomBase64URLRaw(16),
	}, ".")
}

func S560FakeAttestationResultEncoded(contactpoint string) string {
	data := base64.RawURLEncoding.EncodeToString([]byte(fmt.Sprintf(
		`{"challenge_nonce":"%s","username":"%s"}`,
		uuid.New().String(), contactpoint,
	)))
	return "%7B" +
		l4KvStr("keyHash", S560RandomBase64URLRaw(32)) + comma +
		l4KvStr("data", data) + comma +
		l4KvStr("signature", S560RandomBase64URLRaw(96)) +
		"%7D"
}

// buildRegisterBody sinh form-urlencoded body cho S561V4S21 reg.create.account.async.
func buildRegisterBody(profile S560Profile, encPassword, contactpoint, contactpointType, locale string) string {
	variables := createAccountVariablesS560(profile, encPassword, contactpoint, contactpointType)
	...
	parts := []string{
		...
		"locale=" + locale,
		"purpose=fetch",
		"fb_api_req_friendly_name=" + S560FriendlyName,
...
func createAccountVariablesS560(profile S560Profile, encPassword, contactpoint, contactpointType string) string {
	...
	regInfoFields := []string{
		...
		l4KvRaw("attestation_result", S560FakeAttestationResultEncoded(contactpoint)),
	...
	// Capture 561v2 mới: theme_params = [XMDS three_neutral_gray, FDS empty], is_push_on = true.
	return "%7B%22params%22%3A%7B%22params%22%3A%22%7B%5C%22params%5C%22%3A%5C%22%7B" +
		...
		"%5C%5C%5C%22aac%5C%5C%5C%22%3A%5C%5C%5C%22" + aacEncoded + "%5C%5C%5C%22%2C" +
		...
		"%5C%22%7D%22%2C%22bloks_versioning_id%22%3A%22" + S560BloksVer + "%22%2C%22app_id%22%3A%22com.bloks.www.bloks.caa.reg.create.account.async%22%7D%2C%22scale%22%3A%223%22%2C%22nt_context%22%3A%7B%22using_white_navbar%22%3Atrue%2C%22styles_id%22%3A%22" + S560StylesID + "%22%2C%22pixel_ratio%22%3A3%2C%22is_push_on%22%3Atrue%2C%22debug_tooling_metadata_token%22%3Anull%2C%22is_flipper_enabled%22%3Afalse%2C%22theme_params%22%3A%5B%7B%22value%22%3A%5B%22three_neutral_gray%22%5D%2C%22design_system_name%22%3A%22XMDS%22%7D%2C%7B%22value%22%3A%5B%5D%2C%22design_system_name%22%3A%22FDS%22%7D%5D%2C%22bloks_version%22%3A%22" + S560BloksVer + "%22%7D%7D"
}
