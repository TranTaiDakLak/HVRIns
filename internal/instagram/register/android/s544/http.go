// http.go — s544 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s544 tls client: %w", err)
	}
...
// buildHeaders — s544 header format (captured traffic — identical với S559).
func buildHeaders(profile s544Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s544UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s544FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s544OAuthToken},
		[2]string{"x-meta-zca", s544MetaZCA},
