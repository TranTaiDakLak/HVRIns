// profile.go — s544 device profile + UA builder + cookie seed + shared pool.
package s544
...
// s544Profile holds all s544-specific data for one registration session.
type s544Profile struct {
...
// BuildProfileForPlatform builds a complete s544 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s544Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s544Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s544 luôn ra
	// FBAV/544.0.0.60.112 bất kể có file Config riêng hay không.
