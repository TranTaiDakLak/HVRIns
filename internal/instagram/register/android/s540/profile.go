// profile.go — s540 device profile + UA builder + cookie seed + shared pool.
package s540
...
// s540Profile holds all s540-specific data for one registration session.
type s540Profile struct {
...
// BuildProfileForPlatform builds a complete s540 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s540Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s540Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s540 luôn ra
	// FBAV/540.0.0.50.82 bất kể có file Config riêng hay không.
