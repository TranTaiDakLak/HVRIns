// http.go — s540 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s540 tls client: %w", err)
	}
...
// buildHeaders — s540 header format (captured traffic — identical với S559).
func buildHeaders(profile s540Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s540UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s540FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s540OAuthToken},
		[2]string{"x-meta-zca", s540MetaZCA},
