// http.go — s537 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s537 tls client: %w", err)
	}
...
// buildHeaders — s537 header format (captured traffic — identical với S559).
func buildHeaders(profile s537Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s537UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s537FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s537OAuthToken},
		[2]string{"x-meta-zca", s537MetaZCA},
