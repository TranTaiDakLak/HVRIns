// profile.go — s537 device profile + UA builder + cookie seed + shared pool.
package s537
...
// s537Profile holds all s537-specific data for one registration session.
type s537Profile struct {
...
// BuildProfileForPlatform builds a complete s537 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s537Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s537Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s537 luôn ra
	// FBAV/537.0.0.47.77 bất kể có file Config riêng hay không.
