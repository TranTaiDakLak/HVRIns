// Package s545 registers the FB4A S545 create.account.async variant.
package s545
...
func (r *Registerer) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	platform := r.Platform
	if platform == "" {
		platform = defaultSpec().Platform
	}
	return registerAccount(ctx, platform, input, onStatus)
}

func init() {
	instagram.RegisterPlatformRegisterer(Platform, func() instagram.Registerer {
		return &Registerer{Platform: Platform}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	return newWorkerContext(Platform, proxyStr, countryCode)
}

func newWorkerContext(platform, proxyStr, countryCode string) (*WorkerContext, error) {
	spec, ok := SpecForPlatform(platform)
	if !ok {
		return nil, fmt.Errorf("unsupported s54x platform %q", platform)
	}
	sess, err := newSession(proxyStr)
	if err != nil {
		return nil, err
	}
	profile := BuildProfileForPlatform(spec.Platform, countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: spec.Platform, countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Profile() S54xProfile { return w.profile }

func (w *WorkerContext) UserAgent() string {
	if w == nil {
		return ""
	}
	return w.profile.S54xUA
}
...
func (w *WorkerContext) SetUAOptions(addVirtualSpecs bool) {
	...
	locale := w.profile.Locale
	connType := w.profile.ConnType
	w.profile = BuildProfileForPlatformWithUA(w.platform, w.countryCode, addVirtualSpecs)
	if locale != "" {
		w.profile.Locale = locale
	}
	if connType != "" {
		w.profile.ConnType = connType
		w.profile.ConnectionType = connType
	}
}
...
func registerAccount(ctx context.Context, platform string, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	wctx, err := newWorkerContext(platform, proxyStr, countryCode)
	if err != nil {
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if input != nil {
		...
		if input.UserAgent != "" {
			...
			profile.S54xUA = input.UserAgent
		}
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s545] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s545] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s545] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s545] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s545] Password encrypted")
	...
	notify(fmt.Sprintf("[s545] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s545] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s545] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s545] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s545] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S545 %s)", parsed.UID, profile.Device.Model),
		UserAgent:      profile.S54xUA,
