// profile.go — s516 device profile + UA builder + cookie seed + shared pool.
package s516
...
// s516Profile holds all s516-specific data for one registration session.
type s516Profile struct {
...
// BuildProfileForPlatform builds a complete s516 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s516Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s516Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s516 luôn ra
	// FBAV/516.0.0.66.94 bất kể có file Config riêng hay không.
