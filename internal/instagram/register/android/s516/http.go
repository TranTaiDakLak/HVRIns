// http.go — s516 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s516 tls client: %w", err)
	}
...
// buildHeaders — s516 header format (captured traffic — identical với S559).
func buildHeaders(profile s516Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s516UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s516FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s516OAuthToken},
		[2]string{"x-meta-zca", s516MetaZCA},
