// profile.go — S555 device profile + UA builder + cookie seed + shared pool.
package s555
...
// S555Profile holds all S555-specific data for one registration session.
type S555Profile struct {
	fakeinfo.FullRegProfile        // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	S555UA                  string    // S555-specific User-Agent (FBAV 555)
...
// BuildProfileForPlatform builds a complete S555 registration profile (default UA).
// Wrapper backward compat — dùng BuildProfileForPlatformWithUA cho options chi tiết.
func BuildProfileForPlatform(platform, countryCode string) S555Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S555Profile {
