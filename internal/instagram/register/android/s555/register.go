// Package s555 — Samsung Galaxy S23 + Facebook API 555 registration.
// Platform "s555": FBAV/555.0.0.49.59, bloks_versioning_id v555, gzip body,
// updated headers (appnetsession, tasos, priority, qpl-active-flows).
//
// KHÔNG đụng đến register/s23 hoặc register/s557 — đây là platform riêng biệt.
package s555
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS555, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s555", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s555", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Profile() S555Profile { return w.profile }
func (w *WorkerContext) UserAgent() string     { return w.profile.S555UA }
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if input != nil {
		...
		if input.UserAgent != "" {
			...
			profile.S555UA = input.UserAgent
		}
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s555] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s555] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s555] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s555] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s555] Password encrypted")
	...
	notify(fmt.Sprintf("[s555] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s555] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s555] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s555] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s555] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S555 %s)", parsed.UID, profile.Device.Model),
		UserAgent:      profile.S555UA,
