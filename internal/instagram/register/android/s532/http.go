// http.go — s532 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s532 tls client: %w", err)
	}
...
// buildHeaders — s532 header format (captured traffic — identical với S559).
func buildHeaders(profile s532Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s532UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s532FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s532OAuthToken},
		[2]string{"x-meta-zca", s532MetaZCA},
