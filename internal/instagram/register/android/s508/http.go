// http.go — s508 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s508 tls client: %w", err)
	}
...
// buildHeaders — s508 header format (captured traffic — identical với S559).
func buildHeaders(profile s508Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s508UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s508FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s508OAuthToken},
		[2]string{"x-meta-zca", s508MetaZCA},
