// profile.go — s508 device profile + UA builder + cookie seed + shared pool.
package s508
...
// s508Profile holds all s508-specific data for one registration session.
type s508Profile struct {
...
// BuildProfileForPlatform builds a complete s508 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s508Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s508Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s508 luôn ra
	// FBAV/508.0.0.56.102 bất kể có file Config riêng hay không.
