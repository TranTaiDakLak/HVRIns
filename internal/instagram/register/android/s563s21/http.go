// http.go — S563S21 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create S563S21 tls client: %w", err)
	}
...
// buildHeaders — S563S21 header format (port từ s562v3, build .26).
