// profile.go — S563V3S21 device profile + UA builder + cookie seed + shared pool.
...
// S560Profile holds all S563V3S21-specific data for one registration session.
type S560Profile struct {
	...
	S560UA                  string    // S563V3S21-specific User-Agent (FBAV 563, SM-G996B)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S563V3S21 registration profile (default UA).
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S560Profile {
	...
	ua := buildS563V3S21UA(device, locale, carrier, addVirtualSpecs, r)
...
// buildS563V3S21UA — FB4A native UA cho SM-G996B (Galaxy S21+), version đọc từ versions_and_builds.txt.
func buildS563V3S21UA(device S23Device, locale, carrier string, addVirtualSpecs bool, r *rand.Rand) string {
	fbav, fbbv := "563.0.0.0.48", "974373688"
