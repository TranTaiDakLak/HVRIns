// http.go — s519 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s519 tls client: %w", err)
	}
...
// buildHeaders — s519 header format (captured traffic — identical với S559).
func buildHeaders(profile s519Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s519UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s519FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s519OAuthToken},
		[2]string{"x-meta-zca", s519MetaZCA},
