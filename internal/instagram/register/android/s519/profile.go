// profile.go — s519 device profile + UA builder + cookie seed + shared pool.
package s519
...
// s519Profile holds all s519-specific data for one registration session.
type s519Profile struct {
...
// BuildProfileForPlatform builds a complete s519 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s519Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s519Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s519 luôn ra
	// FBAV/519.0.0.68.92 bất kể có file Config riêng hay không.
