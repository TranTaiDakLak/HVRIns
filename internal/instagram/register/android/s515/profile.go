// profile.go — s515 device profile + UA builder + cookie seed + shared pool.
package s515
...
// s515Profile holds all s515-specific data for one registration session.
type s515Profile struct {
...
// BuildProfileForPlatform builds a complete s515 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s515Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s515Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s515 luôn ra
	// FBAV/515.0.0.65.95 bất kể có file Config riêng hay không.
