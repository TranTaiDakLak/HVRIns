// http.go — s515 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s515 tls client: %w", err)
	}
...
// buildHeaders — s515 header format (captured traffic — identical với S559).
func buildHeaders(profile s515Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s515UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s515FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s515OAuthToken},
		[2]string{"x-meta-zca", s515MetaZCA},
