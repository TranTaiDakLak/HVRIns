// http.go — s502 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s502 tls client: %w", err)
	}
...
// buildHeaders — s502 header format (captured traffic — identical với S559).
func buildHeaders(profile s502Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s502UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s502FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s502OAuthToken},
		[2]string{"x-meta-zca", s502MetaZCA},
