// profile.go — s502 device profile + UA builder + cookie seed + shared pool.
package s502
...
// s502Profile holds all s502-specific data for one registration session.
type s502Profile struct {
...
// BuildProfileForPlatform builds a complete s502 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s502Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s502Profile {
	...
	// pick random từ Config/Fbapp/versions_and_builds.txt (file generic chứa 2320 entries
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s502 luôn ra
	// FBAV/502.0.0.44.108 bất kể có file Config riêng hay không.
