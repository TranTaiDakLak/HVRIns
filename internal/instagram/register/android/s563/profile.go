// profile.go â€” S563 device profile + UA builder + cookie seed + shared pool.
package s563
...
// S563Profile holds all S563-specific data for one registration session.
type S563Profile struct {
	fakeinfo.FullRegProfile           // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	S563UA                  string    // S563-specific User-Agent (FBAV 563)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S563 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S563Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S563Profile {
