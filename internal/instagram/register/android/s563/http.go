// http.go â€” S563 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s563 tls client: %w", err)
	}
...
// buildHeaders â€” S563 header format (captured traffic).
// KhÃ¡c s557: bá» network-props/appnet/tasos/qpl; thÃªm rmd/zero-eh/zero-state.
func buildHeaders(profile S563Profile) [][2]string {
	...
	// x-fb-integrity-machine-id conditional (khi cÃ³ MachineID/datr)
	if profile.MachineID != "" {
		h = append(h, [2]string{"x-fb-integrity-machine-id", profile.MachineID})
	}

	h = append(h,
		[2]string{"x-zero-eh", randomHex32()},
		[2]string{"user-agent", profile.S563UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s563FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s563OAuthToken},
		[2]string{"x-meta-zca", s563MetaZCA},
