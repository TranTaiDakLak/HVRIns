// profile.go — s535 device profile + UA builder + cookie seed + shared pool.
package s535
...
// s535Profile holds all s535-specific data for one registration session.
type s535Profile struct {
...
// BuildProfileForPlatform builds a complete s535 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s535Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s535Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s535 luôn ra
	// FBAV/535.0.0.49.72 bất kể có file Config riêng hay không.
