// http.go — s535 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s535 tls client: %w", err)
	}
...
// buildHeaders — s535 header format (captured traffic — identical với S559).
func buildHeaders(profile s535Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s535UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s535FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s535OAuthToken},
		[2]string{"x-meta-zca", s535MetaZCA},
