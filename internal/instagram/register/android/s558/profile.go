// profile.go — S558 device profile + UA builder + cookie seed + shared pool.
package s558
...
// S558Profile holds all S558-specific data for one registration session.
type S558Profile struct {
	fakeinfo.FullRegProfile           // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	S558UA                  string    // S558-specific User-Agent (FBAV 558)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S558 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S558Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S558Profile {
