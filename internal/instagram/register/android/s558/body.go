// body.go — S558 register POST body builder + app constants.
// FBAV/558.0.0.70.72, FBBV/953309385.
// Captured May 2026: doc_id/bloks_ver/styles_id mới, is_push_on=true,
...
var S558AppVersions = []struct {
	Version string // FBAV
	Build   string // FBBV
}{
	{"558.0.0.70.72", "953309385"}, // captured traffic — confirmed (May 2026)
}

const (
	S558DocID        = "1199408042594970992837994886"
	S558BloksVer     = "752772a2bf689dac5dc5fab7c56e888fe08e0569bef53cee7890974e5457ec75"
	S558OAuthToken   = "350685531728|62f8ce9f74b12f84c123cc23437a4a32"
	S558MetaZCA      = "empty_token"
	S558FriendlyName = "FbBloksActionRootQuery-com.bloks.www.bloks.caa.reg.create.account.async"
	S558StylesID     = "fefbd1d6b452bcac026b965ec70e37ee"

	// OriginalUA — UA gốc cố định cho platform S558 (SM-S911B, en_GB, Viettel).
	OriginalUA = "[FBAN/FB4A;FBAV/558.0.0.70.72;FBBV/953309385;FBDM/{density=3.0,width=1080,height=2340};FBLC/en_GB;FBRV/0;FBCR/Viettel;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-S911B;FBSV/15;FBOP/1;FBCA/arm64-v8a:;]"
)

// S23Device represents a Samsung S23 device variant (reused for S558)
...
func S558RandomBase64URLRaw(n int) string {
	b := make([]byte, n)
...
func S558AACJSON(ts int64) string {
	return fmt.Sprintf(
		`{"aac_init_timestamp":%d,"aaccs":"%s","aacjid":"%s"}`,
		ts, S558RandomBase64URLRaw(32), uuid.New().String(),
	)
}

func S558EscL3StringValue(v string) string {
	escaped := url.QueryEscape(v)
	escaped = strings.ReplaceAll(escaped, "%22", "%5C%5C%5C%5C%5C%5C%5C%22")
	return strings.ReplaceAll(escaped, "+", "%20")
}

func S558FakeSafetyNetToken(ts int64) string {
	return S558RandomBase64URLRaw(18) + "." + fmt.Sprintf("%d", ts) + "." + S558RandomBase64URLRaw(48)
}

func S558FakePlayIntegrityToken() string {
	return strings.Join([]string{
		"eyJhbGciOiJSU0EtT0FFUCIsImVuYyI6IkExMjhHQ00ifQ",
		S558RandomBase64URLRaw(64),
		S558RandomBase64URLRaw(16),
		S558RandomBase64URLRaw(96),
		S558RandomBase64URLRaw(16),
	}, ".")
}

func S558FakeAttestationResultEncoded(contactpoint string) string {
	data := base64.RawURLEncoding.EncodeToString([]byte(fmt.Sprintf(
		`{"challenge_nonce":"%s","username":"%s"}`,
		uuid.New().String(), contactpoint,
	)))
	return "%7B" +
		l4KvStr("keyHash", S558RandomBase64URLRaw(32)) + comma +
		l4KvStr("data", data) + comma +
		l4KvStr("signature", S558RandomBase64URLRaw(96)) +
		"%7D"
}

// buildRegisterBody sinh form-urlencoded body cho S558 reg.create.account.async.
func buildRegisterBody(profile S558Profile, encPassword, contactpoint, contactpointType, locale string) string {
	variables := createAccountVariablesS558(profile, encPassword, contactpoint, contactpointType)
	...
	parts := []string{
		...
		"locale=" + locale,
		"purpose=fetch",
		"fb_api_req_friendly_name=" + S558FriendlyName,
		"fb_api_caller_class=graphservice",
		"client_doc_id=" + S558DocID,
...
func createAccountVariablesS558(profile S558Profile, encPassword, contactpoint, contactpointType string) string {
	...
	aacEncoded := S558EscL3StringValue(S558AACJSON(ts))
	safetynetToken := S558FakeSafetyNetToken(ts)
	playIntegrityToken := S558FakePlayIntegrityToken()
	...
	regInfoFields := []string{
		...
		l4KvRaw("attestation_result", S558FakeAttestationResultEncoded(contactpoint)),
	...
	// ── 558 client_input_params giữ schema 559v2, hạ FBAV/FBBV về bản 558 ─────────
	return "%7B%22params%22%3A%7B%22params%22%3A%22%7B%5C%22params%5C%22%3A%5C%22%7B" +
		...
		"%5C%22%7D%22%2C%22bloks_versioning_id%22%3A%22" + S558BloksVer + "%22%2C%22app_id%22%3A%22com.bloks.www.bloks.caa.reg.create.account.async%22%7D%2C%22scale%22%3A%223%22%2C%22nt_context%22%3A%7B%22using_white_navbar%22%3Atrue%2C%22styles_id%22%3A%22" + S558StylesID + "%22%2C%22pixel_ratio%22%3A3%2C%22is_push_on%22%3Atrue%2C%22debug_tooling_metadata_token%22%3Anull%2C%22is_flipper_enabled%22%3Afalse%2C%22theme_params%22%3A%5B%7B%22value%22%3A%5B%5D%2C%22design_system_name%22%3A%22FDS%22%7D%5D%2C%22bloks_version%22%3A%22" + S558BloksVer + "%22%7D%7D"
}
