// http.go — S558 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create S558 tls client: %w", err)
	}
...
// buildHeaders — S558 header format (captured traffic).
// Khác s557: bỏ network-props/appnet/tasos/qpl; thêm rmd/zero-eh/zero-state.
func buildHeaders(profile S558Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.S558UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", S558FriendlyName},
		...
		[2]string{"authorization", "OAuth " + S558OAuthToken},
		[2]string{"x-meta-zca", S558MetaZCA},
