// profile.go — s532 device profile + UA builder + cookie seed + shared pool.
package s533
...
// s533Profile holds all s532-specific data for one registration session.
type s533Profile struct {
...
// BuildProfileForPlatform builds a complete s533 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s533Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s533Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s532 luôn ra
	// FBAV/533.0.0.50.84 bất kể có file Config riêng hay không.
