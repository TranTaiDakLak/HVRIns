...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s533 tls client: %w", err)
	}
...
func buildHeaders(profile s533Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s533UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s533FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s533OAuthToken},
		[2]string{"x-meta-zca", s533MetaZCA},
