// http.go — s525 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s525 tls client: %w", err)
	}
...
// buildHeaders — s525 header format (captured traffic — identical với S559).
func buildHeaders(profile s525Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s525UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s525FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s525OAuthToken},
		[2]string{"x-meta-zca", s525MetaZCA},
