// profile.go — s525 device profile + UA builder + cookie seed + shared pool.
package s525
...
// s525Profile holds all s525-specific data for one registration session.
type s525Profile struct {
...
// BuildProfileForPlatform builds a complete s525 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s525Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s525Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s525 luôn ra
	// FBAV/525.0.0.53.51 bất kể có file Config riêng hay không.
