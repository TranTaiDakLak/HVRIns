// body.go - S545-S554 register POST body builder.
package s550
...
// S23Device represents a Samsung S23 device variant (reused for S54x)
...
func S54xRandomBase64URLRaw(n int) string {
	b := make([]byte, n)
...
func S54xAACJSON(ts int64) string {
	return fmt.Sprintf(
		`{"aac_init_timestamp":%d,"aaccs":"%s","aacjid":"%s"}`,
		ts, S54xRandomBase64URLRaw(32), uuid.New().String(),
	)
}

func S54xEscL3StringValue(v string) string {
	escaped := url.QueryEscape(v)
	escaped = strings.ReplaceAll(escaped, "%22", "%5C%5C%5C%5C%5C%5C%5C%22")
	return strings.ReplaceAll(escaped, "+", "%20")
}

func S54xFakeSafetyNetToken(ts int64) string {
	return S54xRandomBase64URLRaw(18) + "." + fmt.Sprintf("%d", ts) + "." + S54xRandomBase64URLRaw(48)
}

func S54xFakePlayIntegrityToken() string {
	return strings.Join([]string{
		"eyJhbGciOiJSU0EtT0FFUCIsImVuYyI6IkExMjhHQ00ifQ",
		S54xRandomBase64URLRaw(64),
		S54xRandomBase64URLRaw(16),
		S54xRandomBase64URLRaw(96),
		S54xRandomBase64URLRaw(16),
	}, ".")
}

func S54xFakeAttestationResultEncoded(contactpoint string) string {
	data := base64.RawURLEncoding.EncodeToString([]byte(fmt.Sprintf(
		`{"challenge_nonce":"%s","username":"%s"}`,
		uuid.New().String(), contactpoint,
	)))
	return "%7B" +
		l4KvStr("keyHash", S54xRandomBase64URLRaw(32)) + comma +
		l4KvStr("data", data) + comma +
		l4KvStr("signature", S54xRandomBase64URLRaw(96)) +
		"%7D"
}

// buildRegisterBody sinh form-urlencoded body cho S54x reg.create.account.async.
func buildRegisterBody(profile S54xProfile, encPassword, contactpoint, contactpointType, locale string) string {
	variables := createAccountVariablesS54x(profile, encPassword, contactpoint, contactpointType)
	...
	parts := []string{
		...
		"locale=" + locale,
		"purpose=fetch",
		"fb_api_req_friendly_name=" + profile.Spec.FriendlyName,
		"fb_api_caller_class=graphservice",
		"client_doc_id=" + profile.Spec.DocID,
...
func createAccountVariablesS54x(profile S54xProfile, encPassword, contactpoint, contactpointType string) string {
	...
	aacEncoded := S54xEscL3StringValue(S54xAACJSON(ts))
	safetynetToken := S54xFakeSafetyNetToken(ts)
	playIntegrityToken := S54xFakePlayIntegrityToken()
	...
	regInfoFields := []string{
		...
		l4KvRaw("attestation_result", S54xFakeAttestationResultEncoded(contactpoint)),
	...
	return "%7B%22params%22%3A%7B%22params%22%3A%22%7B%5C%22params%5C%22%3A%5C%22%7B" +
		...
		"%5C%22%7D%22%2C%22bloks_versioning_id%22%3A%22" + profile.Spec.BloksVer + "%22%2C%22app_id%22%3A%22com.bloks.www.bloks.caa.reg.create.account.async%22%7D%2C%22scale%22%3A%223%22%2C%22nt_context%22%3A%7B%22using_white_navbar%22%3Atrue%2C%22styles_id%22%3A%22" + profile.Spec.StylesID + "%22%2C%22pixel_ratio%22%3A3%2C%22is_push_on%22%3A" + boolLiteral(profile.Spec.IsPushOn) + "%2C%22debug_tooling_metadata_token%22%3Anull%2C%22is_flipper_enabled%22%3Afalse%2C%22theme_params%22%3A" + profile.Spec.ThemeParams + "%2C%22bloks_version%22%3A%22" + profile.Spec.BloksVer + "%22%7D%7D"
}
