// profile.go — s543 device profile + UA builder + cookie seed + shared pool.
package s543
...
// s543Profile holds all s543-specific data for one registration session.
type s543Profile struct {
...
// BuildProfileForPlatform builds a complete s543 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s543Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s543Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s543 luôn ra
	// FBAV/543.0.0.55.73 bất kể có file Config riêng hay không.
