// http.go — s543 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s543 tls client: %w", err)
	}
...
// buildHeaders — s543 header format (captured traffic — identical với S559).
func buildHeaders(profile s543Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s543UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s543FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s543OAuthToken},
		[2]string{"x-meta-zca", s543MetaZCA},
