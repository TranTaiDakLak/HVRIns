// profile.go — s528 device profile + UA builder + cookie seed + shared pool.
package s528
...
// s528Profile holds all s528-specific data for one registration session.
type s528Profile struct {
...
// BuildProfileForPlatform builds a complete s528 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s528Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s528Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s528 luôn ra
	// FBAV/528.0.0.76.84 bất kể có file Config riêng hay không.
