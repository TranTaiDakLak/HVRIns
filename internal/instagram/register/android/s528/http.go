// http.go — s528 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s528 tls client: %w", err)
	}
...
// buildHeaders — s528 header format (captured traffic — identical với S559).
func buildHeaders(profile s528Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s528UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s528FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s528OAuthToken},
		[2]string{"x-meta-zca", s528MetaZCA},
