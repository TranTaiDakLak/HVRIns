// package s561 — Samsung Galaxy S23 + Facebook API 561 registration.
// Platform "s561": FBAV/561.0.0.3.67, new bloks_versioning_id/doc_id, gzip body,
// bỏ appnetsession/tasos/qpl headers, thêm x-fb-rmd/x-zero-eh/x-zero-state,
// theme_params=[XMDS three_neutral_gray + FDS empty], is_push_on=true.
//
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS561, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s561", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s561", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s561] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s561] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s561] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s561] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s561] Password encrypted")
	...
	notify(fmt.Sprintf("[s561] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s561] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s561] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s561] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s561] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
