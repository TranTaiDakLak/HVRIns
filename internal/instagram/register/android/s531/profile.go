// profile.go — s531 device profile + UA builder + cookie seed + shared pool.
package s531
...
// s531Profile holds all s531-specific data for one registration session.
type s531Profile struct {
...
// BuildProfileForPlatform builds a complete s531 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s531Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s531Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s531 luôn ra
	// FBAV/531.0.0.47.70 bất kể có file Config riêng hay không.
