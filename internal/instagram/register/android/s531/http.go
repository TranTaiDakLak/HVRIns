// http.go — s531 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s531 tls client: %w", err)
	}
...
// buildHeaders — s531 header format (captured traffic — identical với S559).
func buildHeaders(profile s531Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s531UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s531FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s531OAuthToken},
		[2]string{"x-meta-zca", s531MetaZCA},
