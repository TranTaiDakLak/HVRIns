// http.go — s500 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s500 tls client: %w", err)
	}
...
// buildHeaders — s500 header format (captured traffic — identical với S559).
func buildHeaders(profile s500Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s500UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s500FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s500OAuthToken},
		[2]string{"x-meta-zca", s500MetaZCA},
