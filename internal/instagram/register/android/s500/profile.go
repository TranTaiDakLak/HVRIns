// profile.go — s500 device profile + UA builder + cookie seed + shared pool.
package s500
...
// s500Profile holds all s500-specific data for one registration session.
type s500Profile struct {
...
// BuildProfileForPlatform builds a complete s500 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s500Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s500Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s500 luôn ra
	// FBAV/500.0.0.41.110 bất kể có file Config riêng hay không.
