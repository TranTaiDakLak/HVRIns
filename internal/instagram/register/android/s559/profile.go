// profile.go — S559 device profile + UA builder + cookie seed + shared pool.
package s559
...
// S559Profile holds all S559-specific data for one registration session.
type S559Profile struct {
	fakeinfo.FullRegProfile         // embed base profile
	Device   S23Device              // device variant (S23Device defined in body.go)
	S559UA   string                 // S559-specific User-Agent (FBAV 559)
	ConnType string                 // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S559 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S559Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S559Profile {
