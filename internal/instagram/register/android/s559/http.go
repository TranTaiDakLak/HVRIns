// http.go — S559 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s559 tls client: %w", err)
	}
...
// buildHeaders — S559 header format (captured traffic).
// Khác s557: bỏ network-props/appnet/tasos/qpl; thêm rmd/zero-eh/zero-state.
func buildHeaders(profile S559Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.S559UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s559FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s559OAuthToken},
		[2]string{"x-meta-zca", s559MetaZCA},
