// profile.go — s524 device profile + UA builder + cookie seed + shared pool.
package s524
...
// s524Profile holds all s524-specific data for one registration session.
type s524Profile struct {
...
// BuildProfileForPlatform builds a complete s524 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s524Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s524Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s524 luôn ra
	// FBAV/524.0.0.73.87 bất kể có file Config riêng hay không.
