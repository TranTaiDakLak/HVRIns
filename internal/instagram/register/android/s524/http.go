// http.go — s524 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s524 tls client: %w", err)
	}
...
// buildHeaders — s524 header format (captured traffic — identical với S559).
func buildHeaders(profile s524Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s524UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s524FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s524OAuthToken},
		[2]string{"x-meta-zca", s524MetaZCA},
