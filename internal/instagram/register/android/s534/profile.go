// profile.go — s534 device profile + UA builder + cookie seed + shared pool.
package s534
...
// s534Profile holds all s534-specific data for one registration session.
type s534Profile struct {
...
// BuildProfileForPlatform builds a complete s534 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s534Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s534Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s534 luôn ra
	// FBAV/534.0.0.56.76 bất kể có file Config riêng hay không.
