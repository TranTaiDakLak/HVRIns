// http.go — s534 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s534 tls client: %w", err)
	}
...
// buildHeaders — s534 header format (captured traffic — identical với S559).
func buildHeaders(profile s534Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s534UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s534FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s534OAuthToken},
		[2]string{"x-meta-zca", s534MetaZCA},
