// package s562v4s23 — Samsung Galaxy S23 (SM-S911B) + Facebook API 561 v2 registration.
// Platform "s562v4s23": FBAV/561.0.0.42.67, FBBV/976056141 — captured 2026-05.
// Cùng doc_id/bloks_ver như s561v4s21 nhưng device pool S23 (density 3.0, 1080x2340).
// Headers giống s562 (bỏ appnetsession/tasos/qpl, thêm x-fb-rmd/x-zero-eh/x-zero-state),
// theme_params=XMDS+FDS, is_push_on=true. Clone từ register/s562 — swap constants + theme.
//
// KHÔNG đụng đến register/s561, register/s561v2, register/s561v3, register/s562 — riêng biệt.
package s562v4s23
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS562V4S23, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s562v4s23", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s562v4s23", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s562v4s23] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s562v4s23] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s562v4s23] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s562v4s23] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s562v4s23] Password encrypted")
	...
	notify(fmt.Sprintf("[s562v4s23] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s562v4s23] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s562v4s23] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s562v4s23] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s562v4s23] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S562V4S23 561.0.0.42.67 %s)", parsed.UID, profile.Device.Model),
