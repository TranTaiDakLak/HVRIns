// profile.go — s520 device profile + UA builder + cookie seed + shared pool.
package s520
...
// s520Profile holds all s520-specific data for one registration session.
type s520Profile struct {
...
// BuildProfileForPlatform builds a complete s520 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s520Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s520Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s520 luôn ra
	// FBAV/520.0.0.69.91 bất kể có file Config riêng hay không.
