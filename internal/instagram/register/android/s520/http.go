// http.go — s520 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s520 tls client: %w", err)
	}
...
// buildHeaders — s520 header format (captured traffic — identical với S559).
func buildHeaders(profile s520Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s520UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s520FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s520OAuthToken},
		[2]string{"x-meta-zca", s520MetaZCA},
