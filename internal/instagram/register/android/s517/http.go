// http.go — s517 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s517 tls client: %w", err)
	}
...
// buildHeaders — s517 header format (captured traffic — identical với S559).
func buildHeaders(profile s517Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s517UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s517FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s517OAuthToken},
		[2]string{"x-meta-zca", s517MetaZCA},
