// profile.go — s517 device profile + UA builder + cookie seed + shared pool.
package s517
...
// s517Profile holds all s517-specific data for one registration session.
type s517Profile struct {
...
// BuildProfileForPlatform builds a complete s517 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s517Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s517Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s517 luôn ra
	// FBAV/517.0.0.67.93 bất kể có file Config riêng hay không.
