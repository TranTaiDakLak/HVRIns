// http.go — s514 HTTP transport + request headers.
// Identical với S559 — chỉ khác app constants.
package s514
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s514 tls client: %w", err)
	}
...
// buildHeaders — s514 header format (captured traffic — identical với S559).
func buildHeaders(profile s514Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s514UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s514FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s514OAuthToken},
		[2]string{"x-meta-zca", s514MetaZCA},
