// profile.go — s518 device profile + UA builder + cookie seed + shared pool.
package s518
...
// s518Profile holds all s518-specific data for one registration session.
type s518Profile struct {
...
// BuildProfileForPlatform builds a complete s518 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s518Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s518Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s518 luôn ra
	// FBAV/518.0.0.63.86 bất kể có file Config riêng hay không.
