// http.go — s518 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s518 tls client: %w", err)
	}
...
// buildHeaders — s518 header format (captured traffic — identical với S559).
func buildHeaders(profile s518Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s518UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s518FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s518OAuthToken},
		[2]string{"x-meta-zca", s518MetaZCA},
