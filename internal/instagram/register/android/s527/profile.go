// profile.go — s527 device profile + UA builder + cookie seed + shared pool.
package s527
...
// s527Profile holds all s527-specific data for one registration session.
type s527Profile struct {
...
// BuildProfileForPlatform builds a complete s527 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s527Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s527Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s527 luôn ra
	// FBAV/527.0.0.75.85 bất kể có file Config riêng hay không.
