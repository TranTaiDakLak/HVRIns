// http.go — s527 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s527 tls client: %w", err)
	}
...
// buildHeaders — s527 header format (captured traffic — identical với S559).
func buildHeaders(profile s527Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s527UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s527FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s527OAuthToken},
		[2]string{"x-meta-zca", s527MetaZCA},
