// http.go — s504 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s504 tls client: %w", err)
	}
...
// buildHeaders — s504 header format (captured traffic — identical với S559).
func buildHeaders(profile s504Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s504UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s504FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s504OAuthToken},
		[2]string{"x-meta-zca", s504MetaZCA},
