// profile.go — s504 device profile + UA builder + cookie seed + shared pool.
package s504
...
// s504Profile holds all s504-specific data for one registration session.
type s504Profile struct {
...
// BuildProfileForPlatform builds a complete s504 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s504Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s504Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s504 luôn ra
	// FBAV/504.0.0.48.106 bất kể có file Config riêng hay không.
