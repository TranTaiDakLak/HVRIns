// profile.go — S564V1S21 device profile + UA builder + cookie seed + shared pool.
// Device: Samsung Galaxy S23+ (SM-S911B), density 3.0, 1080x2400.
...
// S560Profile holds all S564V1S21-specific data for one registration session.
type S560Profile struct {
	...
	S560UA                  string    // S564V1S21-specific User-Agent (FBAV 563, SM-S911B)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S564V1S21 registration profile (default UA).
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S560Profile {
	...
	ua := buildS564V1S21UA(device, locale, carrier, addVirtualSpecs, r)
...
// buildS564V1S21UA — FB4A native UA cho SM-S911B (Galaxy S23+), version đọc từ versions_and_builds.txt.
func buildS564V1S21UA(device S23Device, locale, carrier string, addVirtualSpecs bool, r *rand.Rand) string {
	fbav, fbbv := "564.0.0.48.74", "986612294"
