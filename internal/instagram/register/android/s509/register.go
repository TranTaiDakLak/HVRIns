// Package s509 — Samsung Galaxy S23 + Facebook API 560 registration.
// Platform "s509": FBAV/509.0.0.58.101, doc_id và bloks_versioning_id mới.
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS509, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s509", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s509", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Profile() s509Profile { return w.profile }

func (w *WorkerContext) UserAgent() string {
	if w == nil {
		return ""
	}
	return w.profile.s509UA
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if input != nil {
		...
		if input.UserAgent != "" {
			...
			profile.s509UA = input.UserAgent
		}
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s509] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s509] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s509] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s509] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s509] Password encrypted")
	...
	notify(fmt.Sprintf("[s509] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s509] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s509] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s509] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s509] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (s509 %s)", parsed.UID, profile.Device.Model),
		UserAgent:      profile.s509UA,
