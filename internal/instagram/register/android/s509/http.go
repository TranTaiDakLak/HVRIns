// http.go — s509 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s509 tls client: %w", err)
	}
...
// buildHeaders — s509 header format (captured traffic — identical với S559).
func buildHeaders(profile s509Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s509UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s509FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s509OAuthToken},
		[2]string{"x-meta-zca", s509MetaZCA},
