// profile.go — s509 device profile + UA builder + cookie seed + shared pool.
package s509
...
// s509Profile holds all s509-specific data for one registration session.
type s509Profile struct {
...
// BuildProfileForPlatform builds a complete s509 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s509Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s509Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s509 luôn ra
	// FBAV/509.0.0.58.101 bất kể có file Config riêng hay không.
