// profile.go — s512 device profile + UA builder + cookie seed + shared pool.
package s512
...
// s512Profile holds all s512-specific data for one registration session.
type s512Profile struct {
...
// BuildProfileForPlatform builds a complete s512 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s512Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s512Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s512 luôn ra
	// FBAV/512.0.0.62.98 bất kể có file Config riêng hay không.
