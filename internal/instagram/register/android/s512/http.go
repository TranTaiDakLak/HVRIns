// http.go — s512 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s512 tls client: %w", err)
	}
...
// buildHeaders — s512 header format (captured traffic — identical với S559).
func buildHeaders(profile s512Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s512UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s512FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s512OAuthToken},
		[2]string{"x-meta-zca", s512MetaZCA},
