// http.go — s542 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s542 tls client: %w", err)
	}
...
// buildHeaders — s542 header format (captured traffic — identical với S559).
func buildHeaders(profile s542Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s542UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s542FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s542OAuthToken},
		[2]string{"x-meta-zca", s542MetaZCA},
