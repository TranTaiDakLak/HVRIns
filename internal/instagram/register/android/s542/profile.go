// profile.go — s542 device profile + UA builder + cookie seed + shared pool.
package s542
...
// s542Profile holds all s542-specific data for one registration session.
type s542Profile struct {
...
// BuildProfileForPlatform builds a complete s542 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s542Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s542Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s542 luôn ra
	// FBAV/542.0.0.60.83 bất kể có file Config riêng hay không.
