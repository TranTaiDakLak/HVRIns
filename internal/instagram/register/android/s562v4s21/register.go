// package s562v4s21 — Samsung Galaxy S21+ (SM-G996B) + Facebook API 561 v2 registration.
// Platform "s562v4s21": FBAV/561.0.0.42.67, FBBV/976056141, captured 2026-05 từ thiết bị S21+.
// Cùng doc_id/bloks_ver như s561v3 verify, theme XMDS+FDS, is_push_on=true.
// Device SM-G996B (density 2.8125, 1080x2400). Clone từ register/s563s21.
//
// KHÔNG đụng đến register/s561, register/s561v2, register/s561v3 — đây là platform riêng biệt.
package s562v4s21
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS562V4S21, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s562v4s21", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s562v4s21", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if input != nil {
		...
		// UseOriginalUA: force device/locale/SIM khớp với OriginalUA (SM-G996B, en_GB, samsung
		// + FBCR carrier do caller chọn). Tránh fingerprint mismatch giữa UA (FBCR/FBLC) và
		// body/headers (Sim.HNI/Locale).
		if input.UseOriginalUA {
			...
			profile.FullRegProfile.Device.Model = "SM-G996B"
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s562v4s21] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s562v4s21] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s562v4s21] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s562v4s21] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s562v4s21] Password encrypted")
	...
	notify(fmt.Sprintf("[s562v4s21] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s562v4s21] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s562v4s21] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s562v4s21] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s562v4s21] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S562V4S21 561.0.0.42.67 %s)", parsed.UID, profile.Device.Model),
