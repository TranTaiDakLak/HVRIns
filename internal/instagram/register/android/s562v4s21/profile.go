// profile.go — S562V4S21 device profile + UA builder + cookie seed + shared pool.
...
// S560Profile holds all S562V4S21-specific data for one registration session.
type S560Profile struct {
	...
	S560UA                  string    // S562V4S21-specific User-Agent (FBAV 561, SM-G996B)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S562V4S21 registration profile (default UA).
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S560Profile {
	...
	ua := buildS562V4S21UA(device, locale, carrier, addVirtualSpecs, r)

	base.UserAgent = ua
	base.DeviceGroup = "312" // captured 561v2 S21+ x-fb-device-group
...
// buildS562V4S21UA — FB4A native UA cho SM-G996B (Galaxy S21+), version đọc từ versions_and_builds.txt.
func buildS562V4S21UA(device S23Device, locale, carrier string, addVirtualSpecs bool, r *rand.Rand) string {
	fbav, fbbv := "561.0.0.42.67", "976056141"
