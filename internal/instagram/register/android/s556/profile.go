// profile.go — S556 device profile + UA builder + cookie seed + shared pool.
package s556
...
// S556Profile holds all S556-specific data for one registration session.
type S556Profile struct {
	fakeinfo.FullRegProfile        // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	S556UA                  string    // S556-specific User-Agent (FBAV 556)
	ConnType                string    // WIFI / mobile.LTE
	AppnetSID               string    // x-fb-appnetsession-sid: 32-char hex (pinned per session)
	AppnetNID               string    // x-fb-appnetsession-nid: 32-char hex + ",Wifi"
}

// BuildProfileForPlatform builds a complete S556 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S556Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S556Profile {
	...
	appnetSID := strings.ReplaceAll(uuid.New().String(), "-", "")[:32]
	appnetNID := strings.ReplaceAll(uuid.New().String(), "-", "")[:32] + ",Wifi"

	return S556Profile{
		FullRegProfile: base,
		Device:         device,
		S556UA:         uaRes.UserAgent,
		ConnType:       connType,
		AppnetSID:      appnetSID,
		AppnetNID:      appnetNID,
	}
