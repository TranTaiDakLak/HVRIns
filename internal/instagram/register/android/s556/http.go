// http.go — S556 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s556 tls client: %w", err)
	}
...
func buildHeaders(profile S556Profile) [][2]string {
	...
	h := [][2]string{
		...
		{"user-agent", profile.S556UA},
		{"x-graphql-request-purpose", "fetch"},
		{"x-fb-friendly-name", s556FriendlyName},
