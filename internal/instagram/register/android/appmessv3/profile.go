// profile.go â€” AppMV3 device profile + UA builder + cookie seed + shared pool.
package appmessv3
...
// LoadSharedPool nạp datr từ các file vào SharedPool (cho CLI/test rotate datr per-attempt).
...
// AppMV3Profile holds all S565-specific data for one registration session.
type AppMV3Profile struct {
	fakeinfo.FullRegProfile           // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	AppMV3UA                string    // S565-specific User-Agent (FBAV 563)
	ConnType                string    // WIFI / mobile.LTE
	// Version-specific (từ versions.go theo platform). 530 để FBAV/FBBV rỗng = pool-driven.
	DocID       string
	RenderDocID string
	BloksVer    string
	FBAV        string
...
func BuildProfileForPlatform(platform, countryCode string) AppMV3Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) AppMV3Profile {
	...
	v := verForPlatform(platform)
	return AppMV3Profile{
		FullRegProfile: base,
		Device:         device,
		AppMV3UA:       uaRes.UserAgent,
		ConnType:       connType,
		DocID:          v.docID,
		RenderDocID:    v.renderDocID,
		BloksVer:       v.bloksVer,
		FBAV:           v.fbav,
		FBBV:           v.fbbv,
	}
