// profile.go — s536 device profile + UA builder + cookie seed + shared pool.
package s536
...
// s536Profile holds all s536-specific data for one registration session.
type s536Profile struct {
...
// BuildProfileForPlatform builds a complete s536 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s536Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s536Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s536 luôn ra
	// FBAV/536.0.0.46.77 bất kể có file Config riêng hay không.
