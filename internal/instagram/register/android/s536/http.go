// http.go — s536 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s536 tls client: %w", err)
	}
...
// buildHeaders — s536 header format (captured traffic — identical với S559).
func buildHeaders(profile s536Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s536UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s536FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s536OAuthToken},
		[2]string{"x-meta-zca", s536MetaZCA},
