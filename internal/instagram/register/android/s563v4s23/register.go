// package s563v4s23 — Samsung Galaxy S23 (SM-S911B) + Facebook API 563 registration.
// Platform "s563v4s23": FBAV/563.0.0.0.26, FBBV/972941018, build .26 doc_id/bloks_ver,
// device SM-S911B (density 3.0, 1080x2340). Clone từ register/s562v3.
//
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS563V4S23, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s563v4s23", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s563v4s23", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s563v4s23] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s563v4s23] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s563v4s23] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s563v4s23] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s563v4s23] Password encrypted")
	...
	notify(fmt.Sprintf("[s563v4s23] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s563v4s23] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s563v4s23] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s563v4s23] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s563v4s23] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S563V4S23 %s)", parsed.UID, profile.Device.Model),
