// profile.go — S563V4S23 device profile + UA builder + cookie seed + shared pool.
...
// S560Profile holds all S563V4S23-specific data for one registration session.
type S560Profile struct {
	...
	S560UA                  string    // S563V4S23-specific User-Agent (FBAV 563, SM-S911B)
	ConnType                string    // WIFI / mobile.LTE
}

// BuildProfileForPlatform builds a complete S563V4S23 registration profile (default UA).
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S560Profile {
	...
	ua := buildS563V4S23UA(device, locale, carrier, addVirtualSpecs, r)
...
// buildS563V4S23UA — FB4A native UA cho SM-S911B (Galaxy S23), version đọc từ versions_and_builds.txt.
func buildS563V4S23UA(device S23Device, locale, carrier string, addVirtualSpecs bool, r *rand.Rand) string {
	fbav, fbbv := "563.0.0.23.73", "978036554"
