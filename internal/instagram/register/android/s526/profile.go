// profile.go — s526 device profile + UA builder + cookie seed + shared pool.
package s526
...
// s526Profile holds all s526-specific data for one registration session.
type s526Profile struct {
...
// BuildProfileForPlatform builds a complete s526 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s526Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s526Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s526 luôn ra
	// FBAV/526.0.0.74.86 bất kể có file Config riêng hay không.
