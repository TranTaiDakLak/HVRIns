// http.go — s526 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s526 tls client: %w", err)
	}
...
// buildHeaders — s526 header format (captured traffic — identical với S559).
func buildHeaders(profile s526Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s526UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s526FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s526OAuthToken},
		[2]string{"x-meta-zca", s526MetaZCA},
