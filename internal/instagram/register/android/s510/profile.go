// profile.go — s510 device profile + UA builder + cookie seed + shared pool.
package s510
...
// s510Profile holds all s510-specific data for one registration session.
type s510Profile struct {
...
// BuildProfileForPlatform builds a complete s510 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s510Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s510Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s510 luôn ra
	// FBAV/510.0.0.60.100 bất kể có file Config riêng hay không.
