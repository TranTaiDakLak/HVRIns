// http.go — s510 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s510 tls client: %w", err)
	}
...
// buildHeaders — s510 header format (captured traffic — identical với S559).
func buildHeaders(profile s510Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s510UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s510FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s510OAuthToken},
		[2]string{"x-meta-zca", s510MetaZCA},
