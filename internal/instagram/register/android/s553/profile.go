// profile.go — S54x device profile + UA builder + cookie seed + shared pool.
package s553
...
// S54xProfile holds all S54x-specific data for one registration session.
type S54xProfile struct {
	fakeinfo.FullRegProfile           // embed base profile
	Device                  S23Device // device variant (S23Device defined in body.go)
	S54xUA                  string    // S54x-specific User-Agent
	ConnType                string    // WIFI / mobile.LTE
	Spec                    Spec      // captured constants for this API version
}

// BuildProfileForPlatform builds a complete S54x registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) S54xProfile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) S54xProfile {
	...
	if uaErr != nil {
		uaRes = uabuilder.UABuildResult{
			UserAgent: OriginalUAForPlatform(spec.Platform),
			Brand:     "samsung",
			Model:     "SM-S911B",
			OSVersion: "15",
			Density:   "3.0",
	...
	base.DeviceGroup = spec.DeviceGroup
