// http.go — s503 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s503 tls client: %w", err)
	}
...
// buildHeaders — s503 header format (captured traffic — identical với S559).
func buildHeaders(profile s503Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s503UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s503FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s503OAuthToken},
		[2]string{"x-meta-zca", s503MetaZCA},
