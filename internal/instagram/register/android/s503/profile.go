// profile.go — s503 device profile + UA builder + cookie seed + shared pool.
package s503
...
// s503Profile holds all s503-specific data for one registration session.
type s503Profile struct {
...
// BuildProfileForPlatform builds a complete s503 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s503Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s503Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s503 luôn ra
	// FBAV/503.0.0.46.107 bất kể có file Config riêng hay không.
