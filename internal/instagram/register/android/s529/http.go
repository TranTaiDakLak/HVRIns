// http.go — s529 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s529 tls client: %w", err)
	}
...
// buildHeaders — s529 header format (captured traffic — identical với S559).
func buildHeaders(profile s529Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s529UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s529FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s529OAuthToken},
		[2]string{"x-meta-zca", s529MetaZCA},
