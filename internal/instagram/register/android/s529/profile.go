// profile.go — s529 device profile + UA builder + cookie seed + shared pool.
package s529
...
// s529Profile holds all s529-specific data for one registration session.
type s529Profile struct {
...
// BuildProfileForPlatform builds a complete s529 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s529Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s529Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s529 luôn ra
	// FBAV/529.0.0.77.83 bất kể có file Config riêng hay không.
