// http.go — s511 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s511 tls client: %w", err)
	}
...
// buildHeaders — s511 header format (captured traffic — identical với S559).
func buildHeaders(profile s511Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s511UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s511FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s511OAuthToken},
		[2]string{"x-meta-zca", s511MetaZCA},
