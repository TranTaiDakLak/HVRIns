// profile.go — s511 device profile + UA builder + cookie seed + shared pool.
package s511
...
// s511Profile holds all s511-specific data for one registration session.
type s511Profile struct {
...
// BuildProfileForPlatform builds a complete s511 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s511Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s511Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s511 luôn ra
	// FBAV/511.0.0.61.99 bất kể có file Config riêng hay không.
