// http.go — s521 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s521 tls client: %w", err)
	}
...
// buildHeaders — s521 header format (captured traffic — identical với S559).
func buildHeaders(profile s521Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s521UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s521FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s521OAuthToken},
		[2]string{"x-meta-zca", s521MetaZCA},
