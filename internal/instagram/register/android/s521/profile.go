// profile.go — s521 device profile + UA builder + cookie seed + shared pool.
package s521
...
// s521Profile holds all s521-specific data for one registration session.
type s521Profile struct {
...
// BuildProfileForPlatform builds a complete s521 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s521Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s521Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s521 luôn ra
	// FBAV/521.0.0.70.90 bất kể có file Config riêng hay không.
