// http.go — s505 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s505 tls client: %w", err)
	}
...
// buildHeaders — s505 header format (captured traffic — identical với S559).
func buildHeaders(profile s505Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s505UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s505FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s505OAuthToken},
		[2]string{"x-meta-zca", s505MetaZCA},
