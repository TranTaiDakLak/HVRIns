// profile.go — s505 device profile + UA builder + cookie seed + shared pool.
package s505
...
// s505Profile holds all s505-specific data for one registration session.
type s505Profile struct {
...
// BuildProfileForPlatform builds a complete s505 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s505Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s505Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s505 luôn ra
	// FBAV/505.0.0.50.105 bất kể có file Config riêng hay không.
