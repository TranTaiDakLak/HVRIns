// http.go — s530 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s530 tls client: %w", err)
	}
...
// buildHeaders — s530 header format (captured traffic — identical với S559).
func buildHeaders(profile s530Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s530UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s530FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s530OAuthToken},
		[2]string{"x-meta-zca", s530MetaZCA},
