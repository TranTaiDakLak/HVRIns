// profile.go — s530 device profile + UA builder + cookie seed + shared pool.
package s530
...
// s530Profile holds all s530-specific data for one registration session.
type s530Profile struct {
...
// BuildProfileForPlatform builds a complete s530 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s530Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s530Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s530 luôn ra
	// FBAV/530.0.0.48.74 bất kể có file Config riêng hay không.
