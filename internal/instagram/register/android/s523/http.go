// http.go — s523 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s523 tls client: %w", err)
	}
...
// buildHeaders — s523 header format (captured traffic — identical với S559).
func buildHeaders(profile s523Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s523UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s523FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s523OAuthToken},
		[2]string{"x-meta-zca", s523MetaZCA},
