// profile.go — s523 device profile + UA builder + cookie seed + shared pool.
package s523
...
// s523Profile holds all s523-specific data for one registration session.
type s523Profile struct {
...
// BuildProfileForPlatform builds a complete s523 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s523Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s523Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s523 luôn ra
	// FBAV/523.0.0.72.88 bất kể có file Config riêng hay không.
