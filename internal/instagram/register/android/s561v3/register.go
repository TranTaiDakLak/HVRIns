// package s561v3 — Samsung Galaxy S23 + Facebook API 561 v2 registration.
// Platform "s561v2": FBAV/561.0.0.42.67, FBBV/968460367, doc_id mới so với s561,
// gzip body, bỏ appnetsession/tasos/qpl headers, thêm x-fb-rmd/x-zero-eh/x-zero-state.
//
// KHÔNG đụng đến register/s561 hay register/s560v2 — đây là platform riêng biệt.
package s561v3
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS561V3, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s561v2] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s561v2] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s561v2] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify("[s561v2] Bắt đầu reg")

	notify(fmt.Sprintf("[s561v2] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s561v2] Password encrypted")
	...
	notify(fmt.Sprintf("[s561v2] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s561v2] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s561v2] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s561v2] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s561v2] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S561V2 %s)", parsed.UID, profile.Device.Model),
