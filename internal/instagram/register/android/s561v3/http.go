...
// buildHeaders — S561V3 header format (captured traffic).
// Khác s561v2: bỏ x-fb-rmd, x-zero-eh, x-zero-state, x-zero-f-device-id;
// đổi thứ tự — user-agent lên trước, x-fb-friendly-name ngay sau,
// x-fb-net-hni trước x-fb-sim-hni.
func buildHeaders(profile S560Profile) [][2]string {
	...
	h := [][2]string{
		...
		{"user-agent", profile.S560UA},
		{"x-fb-friendly-name", S560FriendlyName},
	}
	...
	h = append(h,
		...
		[2]string{"x-fb-device-group", profile.DeviceGroup},
