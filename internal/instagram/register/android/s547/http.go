// http.go — S54x HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create S547 tls client: %w", err)
	}
...
// buildHeaders — S54x header format (captured traffic).
// Khác s557: bỏ network-props/appnet/tasos/qpl; thêm rmd/zero-eh/zero-state.
func buildHeaders(profile S54xProfile) [][2]string {
	...
	if profile.Spec.ZeroEH != "" {
		h = append(h, [2]string{"x-zero-eh", profile.Spec.ZeroEH})
	}

	h = append(h,
		[2]string{"user-agent", profile.S54xUA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", profile.Spec.FriendlyName},
		...
		[2]string{"authorization", "OAuth " + profile.Spec.OAuthToken},
		[2]string{"x-meta-zca", profile.Spec.MetaZCA},
