// profile.go — s507 device profile + UA builder + cookie seed + shared pool.
package s507
...
// s507Profile holds all s507-specific data for one registration session.
type s507Profile struct {
...
// BuildProfileForPlatform builds a complete s507 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s507Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s507Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s507 luôn ra
	// FBAV/507.0.0.54.103 bất kể có file Config riêng hay không.
