// http.go — s507 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s507 tls client: %w", err)
	}
...
// buildHeaders — s507 header format (captured traffic — identical với S559).
func buildHeaders(profile s507Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s507UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s507FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s507OAuthToken},
		[2]string{"x-meta-zca", s507MetaZCA},
