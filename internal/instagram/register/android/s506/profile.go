// profile.go — s506 device profile + UA builder + cookie seed + shared pool.
package s506
...
// s506Profile holds all s506-specific data for one registration session.
type s506Profile struct {
...
// BuildProfileForPlatform builds a complete s506 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s506Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s506Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s506 luôn ra
	// FBAV/506.0.0.52.104 bất kể có file Config riêng hay không.
