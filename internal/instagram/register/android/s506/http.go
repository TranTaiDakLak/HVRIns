// http.go — s506 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s506 tls client: %w", err)
	}
...
// buildHeaders — s506 header format (captured traffic — identical với S559).
func buildHeaders(profile s506Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s506UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s506FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s506OAuthToken},
		[2]string{"x-meta-zca", s506MetaZCA},
