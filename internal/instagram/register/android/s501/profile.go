// profile.go — s501 device profile + UA builder + cookie seed + shared pool.
package s501
...
// s501Profile holds all s501-specific data for one registration session.
type s501Profile struct {
...
// BuildProfileForPlatform builds a complete s501 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s501Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s501Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s501 luôn ra
	// FBAV/501.0.0.39.109 bất kể có file Config riêng hay không.
