// http.go — s501 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s501 tls client: %w", err)
	}
...
// buildHeaders — s501 header format (captured traffic — identical với S559).
func buildHeaders(profile s501Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s501UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s501FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s501OAuthToken},
		[2]string{"x-meta-zca", s501MetaZCA},
