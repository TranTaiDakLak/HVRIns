// http.go — s513 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s513 tls client: %w", err)
	}
...
// buildHeaders — s513 header format (captured traffic — identical với S559).
func buildHeaders(profile s513Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s513UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s513FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s513OAuthToken},
		[2]string{"x-meta-zca", s513MetaZCA},
