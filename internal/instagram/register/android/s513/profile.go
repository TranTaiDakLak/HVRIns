// profile.go — s513 device profile + UA builder + cookie seed + shared pool.
package s513
...
// s513Profile holds all s513-specific data for one registration session.
type s513Profile struct {
...
// BuildProfileForPlatform builds a complete s513 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s513Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s513Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s513 luôn ra
	// FBAV/513.0.0.63.97 bất kể có file Config riêng hay không.
