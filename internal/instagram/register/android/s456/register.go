// package s456 — Samsung Galaxy S23 + Facebook API 456 registration.
// Platform "s456": FBAV/456.0.0.0.56, new bloks_versioning_id/doc_id, gzip body,
...
func init() {
	instagram.RegisterPlatformRegisterer(instagram.PlatformS456, func() instagram.Registerer {
		return &Registerer{}
	})
}
...
func NewWorkerContext(proxyStr, countryCode string) (*WorkerContext, error) {
	...
	profile := BuildProfileForPlatform("s456", countryCode)
	return &WorkerContext{sess: sess, profile: profile, platform: "s456", countryCode: countryCode}, nil
}
...
func (w *WorkerContext) Register(ctx context.Context, input *instagram.RegInput, onStatus func(string)) *instagram.RegResult {
	...
	if profile.MachineID == "" && SharedPool != nil {
		if poolDatr := SharedPool.GetNext(slotIdx); poolDatr != "" {
			...
			notify(fmt.Sprintf("[s456] New initial %s (used %d | S/F/U: %d/%d/%d)",
				poolDatr, used, s, f, u))
		} else {
			notify(fmt.Sprintf("[s456] ⚠️ Pool EMPTY (slot=%d) — reg KHÔNG có datr!", slotIdx))
		}
	} else if profile.MachineID != "" && SharedPool != nil {
		s, f, u, used := SharedPool.GetStats(profile.MachineID)
		notify(fmt.Sprintf("[s456] Dùng datr %s | used=%d S/F/U=%d/%d/%d",
	...
	notify(fmt.Sprintf("[s456] Start — %s %s | %s | %s | seed=%s",
	...
	notify("[s456] Password encrypted")
	...
	notify(fmt.Sprintf("[s456] POST graphql (%s%s)...", profile.Device.Model, datrInfo))

	respBody, err := sess.postGzip(ctx, instagram.BaseURLBGraph+"/graphql", body, headers)
	if err != nil {
		notify(fmt.Sprintf("[s456] HTTP error: %v", err))
		if profile.MachineID != "" && SharedPool != nil {
	...
	notify(fmt.Sprintf("[s456] Response (%d bytes)", len(respBody)))
	...
	notify(fmt.Sprintf("[s456] Success! UID=%s Token=%s...", parsed.UID, parsed.AccessToken[:smin(len(parsed.AccessToken), 20)]))

	time.Sleep(time.Duration(1000+rand.Intn(1000)) * time.Millisecond)
	xzeroEh := fetchXZeroEh(ctx, sess, profile, parsed.AccessToken, profile.DeviceID)
	if xzeroEh != "unknown" && xzeroEh != "" {
		notify(fmt.Sprintf("[s456] X-Zero-EH: %s", xzeroEh[:smin(len(xzeroEh), 16)]))
	}
	...
	return &instagram.RegResult{
		...
		Password:       password,
		Message:        fmt.Sprintf("Register OK — UID: %s (S456 %s)", parsed.UID, profile.Device.Model),
