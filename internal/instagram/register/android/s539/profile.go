// profile.go — s539 device profile + UA builder + cookie seed + shared pool.
package s539
...
// s539Profile holds all s539-specific data for one registration session.
type s539Profile struct {
...
// BuildProfileForPlatform builds a complete s539 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s539Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s539Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s539 luôn ra
	// FBAV/539.0.0.54.69 bất kể có file Config riêng hay không.
