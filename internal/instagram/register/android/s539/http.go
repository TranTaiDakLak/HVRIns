// http.go — s539 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s539 tls client: %w", err)
	}
...
// buildHeaders — s539 header format (captured traffic — identical với S559).
func buildHeaders(profile s539Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s539UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s539FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s539OAuthToken},
		[2]string{"x-meta-zca", s539MetaZCA},
