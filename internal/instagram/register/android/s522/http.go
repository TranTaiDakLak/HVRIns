// http.go — s522 HTTP transport + request headers.
...
func newSession(proxyStr string) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create s522 tls client: %w", err)
	}
...
// buildHeaders — s522 header format (captured traffic — identical với S559).
func buildHeaders(profile s522Profile) [][2]string {
	...
	h = append(h,
		...
		[2]string{"user-agent", profile.s522UA},
		[2]string{"x-graphql-request-purpose", "fetch"},
		[2]string{"x-fb-friendly-name", s522FriendlyName},
		...
		[2]string{"authorization", "OAuth " + s522OAuthToken},
		[2]string{"x-meta-zca", s522MetaZCA},
