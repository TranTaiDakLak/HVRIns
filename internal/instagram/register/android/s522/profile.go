// profile.go — s522 device profile + UA builder + cookie seed + shared pool.
package s522
...
// s522Profile holds all s522-specific data for one registration session.
type s522Profile struct {
...
// BuildProfileForPlatform builds a complete s522 registration profile (default UA).
func BuildProfileForPlatform(platform, countryCode string) s522Profile {
...
func BuildProfileForPlatformWithUA(platform, countryCode string, addVirtualSpecs bool) s522Profile {
	...
	// trộn FBAV 300→559+ → có thể trúng version cũ). Hardcode đảm bảo s522 luôn ra
	// FBAV/522.0.0.71.89 bất kể có file Config riêng hay không.
