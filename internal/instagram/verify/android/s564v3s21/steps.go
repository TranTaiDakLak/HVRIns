// steps.go — S564V1S21 verify: thin wrapper delegating to verifybase.RunVerify.
// Captured 564.0.0.48.74, FBBV 986612294, device SM-G996B (Galaxy S21+).
// AddEmail dùng doc_id riêng (addEmailDocID) + friendly name FbBloksAppRootQuery (khác các version cũ).
// Confirm dùng verifyDocID (= doc_id của Reg).
package s564v3s21
...
const (
	verifyDocID     = "11994080422959257612163598103"
	verifyBloksVer  = "ebb84a871fc81d8889c76b8300f0825f5864655c74af6e62e512aedb615e5a80"
	defaultStylesID = "b19b0c9a5b06c330f3ad3eb12dcf0d82"
)
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		SetupSessionCtx: nil, // S564V1S21 does not use appnet/tasos fields
		...
		Enable2FA:         enable2FAForS564V1S21,
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s564v3s21 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s564v3s21 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS564V1S21(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s564v3s21 verify] 2FA secret generated: %s", res.Secret))
