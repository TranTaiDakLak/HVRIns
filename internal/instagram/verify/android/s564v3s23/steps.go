...
// Captured 564.0.0.48.74, FBBV 986612294, device SM-S911B (Galaxy S23+).
	...
	spec := verifybase.Spec{
		...
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s564v2s21 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s564v2s21 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS564V1S21(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s564v2s21 verify] 2FA secret generated: %s", res.Secret))
