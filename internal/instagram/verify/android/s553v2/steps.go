// steps.go — S553V2 verify: thin wrapper delegating to verifybase.RunVerify.
// All shared orchestration logic lives in verifybase. This file contains only
// S560-specific constants, UA validation/builder, body builders, 2FA, and addinfo wiring.
// S560 differs from s558: new doc_id/bloks_ver, styles_id, is_push_on=false.
package s553v2
...
const (
	verifyDocID     = "119940804212296436181797579246"
	verifyBloksVer  = "e6824e76c6ba5c44b62f86b6ae9c904cea8551b02256b18cba10b405e29d8f79"
	defaultStylesID = "cc0995fd4392efdb0fd2bbe7fc9091fe"
)
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		FixUA: func(ua, phone string) (string, string) {
			...
			return newUA, fmt.Sprintf("UA regenerated (FBAV/553, country=%s)", label)
		},
		SetupSessionCtx: nil, // S560 does not use appnet/tasos fields
		...
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s553v2 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s553v2 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS560(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s553v2 verify] 2FA secret generated: %s", res.Secret))
...
// ─── Body builders (new style: DENIED perms, si_device_param_network_info) ───
...
func buildAAC() string {
	return fmt.Sprintf(
		`{"aac_init_timestamp":%d,"aaccs":"%s","aacjid":"%s"}`,
		time.Now().Unix(), randomBase64URLRaw(32), uuid.New().String(),
	)
}

func fakeSafetyNetToken() string {
	return "dW5rbm93bg." + fmt.Sprintf("%d", time.Now().Unix()) + "." + randomBase64URLRaw(48)
}

func fakePlayIntegrityToken() string {
	return strings.Join([]string{
		"eyJhbGciOiJSU0EtT0FFUCIsImVuYyI6IkExMjhHQ00ifQ",
		randomBase64URLRaw(64),
		randomBase64URLRaw(16),
		randomBase64URLRaw(96),
		randomBase64URLRaw(16),
	}, ".")
}

func fakeAttestationResult(username string) map[string]interface{} {
	data := base64.RawURLEncoding.EncodeToString([]byte(fmt.Sprintf(
		`{"challenge_nonce":"%s","username":"%s"}`,
		uuid.New().String(), username,
	)))
	return map[string]interface{}{
		"keyHash":   randomBase64URLRaw(32),
		"data":      data,
		"signature": randomBase64URLRaw(96),
	}
}

func fdsOnlyThemeParams() []interface{} {
	return []interface{}{map[string]interface{}{"value": []string{}, "design_system_name": "FDS"}}
}
...
func buildRegInfo(emailAddr, uid, firstName, lastName, deviceID, familyDevID string, gender int, sim fakeinfo.SimProfile) string {
	...
	ri := map[string]interface{}{
		...
		"headers_flow_id":                       nil,
		"accounts_list_client":                  []interface{}{},
		"fdid_available_on_start":               nil,
		"fdid_rid_available_on_start":           nil,
		"sign_in_with_google_email":             nil,
		"os_shared_age_range":                   nil,
		"safetynet_token":                       fakeSafetyNetToken(),
		"caa_play_integrity_attestation_result": fakePlayIntegrityToken(),
		...
		"attestation_result":                      fakeAttestationResult(emailAddr),
		"device_network_info":                     siDeviceNetworkInfoInt(sim),
...
func buildRegInfoForConfirm(emailAddr, uid, firstName, lastName, deviceID, familyDevID string, gender int, sim fakeinfo.SimProfile) string {
...
func buildAddEmailBody(spec *verifybase.Spec, emailAddr, uid, firstName, lastName, deviceID, familyDevID, waterfallID, machineID, locale string, gender int, sim fakeinfo.SimProfile) string {
	...
	variablesJSON := verifybase.BuildVariablesWithTheme(paramsObj, spec.BloksVer, spec.StylesID, spec.IsPushOn, fdsOnlyThemeParams())
...
func buildConfirmBody(spec *verifybase.Spec, emailAddr, code, uid, firstName, lastName, deviceID, familyDevID, waterfallID, machineID, locale string, gender int, sim fakeinfo.SimProfile) string {
	...
	eventReqID := uuid.New().String()
	latency := int64(80000000000000 + mrand.Int63n(9000000000000))
	regInfo := buildRegInfoForConfirm(emailAddr, uid, firstName, lastName, deviceID, familyDevID, gender, sim)
