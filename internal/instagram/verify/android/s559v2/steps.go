// steps.go — S559 verify: thin wrapper delegating to verifybase.RunVerify.
// All shared orchestration logic lives in verifybase. This file contains only
// s559-specific constants, UA validation/builder, body builders, 2FA, and addinfo wiring.
// s559 differs from s558: new doc_id/bloks_ver, styles_id, is_push_on=false.
package s559v2
...
const (
	verifyDocID     = "1199408042594970992837994886"
	verifyBloksVer  = "752772a2bf689dac5dc5fab7c56e888fe08e0569bef53cee7890974e5457ec75"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		FixUA: func(ua, phone string) (string, string) {
			...
			return newUA, fmt.Sprintf("UA regenerated (FBAV/559, country=%s)", label)
		},
		SetupSessionCtx: nil, // s559 does not use appnet/tasos fields
		...
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s559v2 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s559v2 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS559(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s559v2 verify] 2FA secret generated: %s", res.Secret))
