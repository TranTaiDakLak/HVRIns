// steps.go — S563V4S21 verify: thin wrapper delegating to verifybase.RunVerify.
// All shared orchestration logic lives in verifybase. This file contains only
// S563V4S21-specific constants, UA validation/builder, body builders, 2FA, and addinfo wiring.
// Cùng doc_id/bloks_ver/styles_id với s563v2 (build 563.0.0.0.26), is_push_on=true, theme FDS-only.
// Khác s563v2: device trong UA = SM-G996B (Galaxy S21+, density 2.8125, 1080x2400).
package s563v6s21
...
const (
	verifyDocID     = "11994080426554467581667453023"
	verifyBloksVer  = "f9d474f2985f03adc1635376403e3dd685665c1ad331ef9e08731e66a6f7e77a"
	defaultStylesID = "af54108bceece41cb17f2cc7dbab8d3e"
)
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		FixUA: func(ua, phone string) (string, string) {
			...
			return newUA, fmt.Sprintf("UA regenerated (FBAV/563 S21+, country=%s)", label)
		},
		SetupSessionCtx: nil, // S563V4S21 does not use appnet/tasos fields
		...
		Enable2FA:         enable2FAForS563V4S21,
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s563v6s21 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s563v6s21 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS563V4S21(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s563v6s21 verify] 2FA secret generated: %s", res.Secret))
