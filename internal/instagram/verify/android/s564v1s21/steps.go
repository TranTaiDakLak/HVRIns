...
// Captured 564.0.0.0.17, FBBV 977893103, device SM-G996B (Galaxy S21+).
...
const (
	verifyDocID     = "11994080428908932823877210452"
	verifyBloksVer  = "c36891378e117e5eaef5b658a382d93c08b183fa9495d1facec29fbb62a81f8a"
	defaultStylesID = "998f39d919f970c8b482d16e00c743a4"
)
	...
	spec := verifybase.Spec{
		...
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s564v1s21 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s564v1s21 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS564V1S21(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s564v1s21 verify] 2FA secret generated: %s", res.Secret))
