// steps.go — S23 verify: add email + wait OTP + confirm code + resend.
// All shared logic lives in verifybase. This file contains only s23-specific
// constants, body builders, the header/body wiring, 2FA, and verifyAccount entry-point.
package s23
...
import (
	...
	"HVRIns/internal/instagram/verify/webandroid"
)
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		// S23 does not validate UA version — use whatever is in session.
		FixUA: nil,
		...
		Enable2FA:         enable2FAForS23,
		PostConfirm:       nil,
	}
	return verifybase.RunVerify(ctx, session, cfg, outputPath, onStatus, spec)
}

// enable2FAForS23 enables 2FA via the Chrome Android AccountsCenter flow.
func enable2FAForS23(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	if session.Cookie == "" {
		return "", nil // non-fatal: skip 2FA without cookie
	}
	return webandroid.Enable2FA(ctx, session.Proxy, session.Cookie, uid, session.UserAgent, "", "", emailOTPFn, notify)
}
