// steps.go — S563V3S21 verify: thin wrapper delegating to verifybase.RunVerify.
// All shared orchestration logic lives in verifybase. This file contains only
// S563V3S21-specific constants, UA validation/builder, body builders, 2FA, and addinfo wiring.
...
const (
	verifyDocID     = "11994080422283368733301219073"
	verifyBloksVer  = "aa4e5e759b2f90492ba1b311dc98988f125b76705083a7e8b3fa11a3262f7459"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		SetupSessionCtx: nil, // S563V3S21 does not use appnet/tasos fields
		...
		Enable2FA:         enable2FAForS563V3S21,
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s563v3s21 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s563v3s21 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS563V3S21(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s563v3s21 verify] 2FA secret generated: %s", res.Secret))
