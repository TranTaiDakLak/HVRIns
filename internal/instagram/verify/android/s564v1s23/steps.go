// steps.go — S564V1S23 verify: thin wrapper delegating to verifybase.RunVerify.
// Captured 564.0.0.0.17, FBBV 977893103, device SM-S911B (Galaxy S23).
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		SetupSessionCtx: nil, // S564V1S23 does not use appnet/tasos fields
		...
		Enable2FA:         enable2FAForS564V1S23,
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s564v1s23 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s564v1s23 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS564V1S23(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s564v1s23 verify] 2FA secret generated: %s", res.Secret))
