// steps.go — S555 verify: thin wrapper delegating to verifybase.RunVerify.
// All shared orchestration logic lives in verifybase. This file contains only
// s555-specific constants, UA validation/builder, body builders, 2FA, and addinfo wiring.
package s555
	...
	spec := verifybase.Spec{
		...
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s555 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s555 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS555(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s555 verify] 2FA secret generated: %s", res.Secret))
