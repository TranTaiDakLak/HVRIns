// steps.go — S563V4S23 verify: thin wrapper delegating to verifybase.RunVerify.
// All shared orchestration logic lives in verifybase. This file contains only
// S563V4S23-specific constants, UA validation/builder, body builders, 2FA, and addinfo wiring.
// Cùng doc_id/bloks_ver/styles_id với s563v2 (build 563.0.0.0.26), is_push_on=true, theme FDS-only.
// Khác s563v2: device trong UA = SM-S911B (Galaxy S23, density 3.0, 1080x2340).
package s563v4s23
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	spec := verifybase.Spec{
		...
		SetupSessionCtx: nil, // S563V4S23 does not use appnet/tasos fields
		...
		Enable2FA:         enable2FAForS563V4S23,
		PostConfirm: func(ctx context.Context, sess *instagram.Session, cfg *instagram.VerifyConfig, notify func(string)) {
			if cfg.AddInfo != nil && cfg.AddInfo.Enabled {
				notify("[s563v4s23 verify] Running AddInfo...")
				res := addinfo.RunAddInfo(ctx, sess, cfg.AddInfo, notify)
				if len(res.Notes) > 0 {
					notify(fmt.Sprintf("[s563v4s23 verify] AddInfo done: %s", strings.Join(res.Notes, ", ")))
				}
			}
		},
	}
...
func enable2FAForS563V4S23(ctx context.Context, session *instagram.Session, uid, machineID, deviceID string, emailOTPFn func(string, int) string, notify func(string)) (string, error) {
	...
	notify(fmt.Sprintf("[s563v4s23 verify] 2FA secret generated: %s", res.Secret))
