// steps.go — iOS540 verify: Spec + iOS Bloks CAA body builders + iOS headers.
//
...
const (
	...
	verifyDocID    = "375801096015625552936537488221"
	verifyBloksVer = "2c91e9ef4a009efcc18d92dc806adbac55d426a117fb08404a627cb362b4a7cc"
	verifyStylesID = "ea1f61ece6844e3971250ff5d7fb3ba3"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "540.0.0.44.68"
	fbBuildNum   = "828638047"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS540 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS540 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS540 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		Enable2FA:                nil, // iOS540 chưa hỗ trợ 2FA
		PostConfirm:              nil,
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios540reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
