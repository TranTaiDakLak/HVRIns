...
const (
	...
	verifyDocID    = "37580109606363295402057509977"
	verifyBloksVer = "5b25d9e3179bff5e6095150efe44a59cb76c3361dc6dbd67a523290e389430c1"
	verifyStylesID = "f162ffeea45d2c2dcba7b672510d8569"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "486.0.0.32.109"
	fbBuildNum   = "653297272"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS486 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS486 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS486 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios486reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
