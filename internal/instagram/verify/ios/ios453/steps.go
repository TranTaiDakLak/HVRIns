...
const (
	...
	fbAppVersion = "453.0.0.38.108"
	fbBuildNum   = "571714389"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS453 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS453 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS453 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios453reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
