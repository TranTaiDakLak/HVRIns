// steps.go — iOS562 verify: Spec + iOS Bloks CAA body builders + iOS headers.
//
...
const (
	...
	verifyDocID    = "375801096013313544589153066091"
	verifyBloksVer = "3be43264256070fd9e818b280c6ceee37df0f87b8ae0ed538cf3ec6b1d600d6a"
	verifyStylesID = "2d85de7a219912218014b808f1a34dda"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "563.0.0.67.72"
	fbBuildNum   = "980285082"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS562 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS562 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS562 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		Enable2FA:          nil, // iOS562 chưa hỗ trợ 2FA
		PostConfirm:        nil,
