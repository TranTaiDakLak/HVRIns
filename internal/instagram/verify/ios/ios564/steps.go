// steps.go — iOS564 verify: Spec + iOS Bloks CAA body builders + iOS headers.
//
...
const (
	...
	verifyDocID    = "37580109609106921721904672240"
	verifyBloksVer = "ef89eb5fbecdcbc7ddfaa0804847e4508af71e1558ff3a5e7f4dd6f6efb490f5"
	verifyStylesID = "35bb60d9f4297087b748b80b372c84a8"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "564.0.0.57.71"
	fbBuildNum   = "985438427"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS564 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS564 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS564 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		SetupSessionCtx: func(sc *verifybase.SessionCtx) {
			...
			sc.ConnType = "wifi.CTRadioAccessTechnologyLTE" // TEST: ép wifi.* theo capture (revert: fakeinfo.RandomIOSConnType())
		...
		Enable2FA:                nil, // iOS564 chưa hỗ trợ 2FA
		PostConfirm:              nil,
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios564reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
...
func buildIOSVerifyHeaders(sc *verifybase.SessionCtx, friendlyName string, withZeroState bool) [][2]string {
	...
	if connType == "" {
		connType = "wifi.CTRadioAccessTechnologyLTE"
	}
