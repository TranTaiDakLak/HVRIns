// steps.go — iOS550 verify: Spec + iOS Bloks CAA body builders + iOS headers.
//
...
const (
	...
	verifyDocID    = "375801096011492436262755449392"
	verifyBloksVer = "17af4c1638fc98049e66e494fab4c23c69d895c2c60e4955b62eac57669f7fa8"
	verifyStylesID = "3919a3502c93e2f38507bbcfd88f6b77"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "554.0.0.34.65"
	fbBuildNum   = "917233084"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS554 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS554 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS554 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		Enable2FA:                nil, // iOS550 chưa hỗ trợ 2FA
		PostConfirm:              nil,
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios554reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
