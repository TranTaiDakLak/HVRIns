...
const (
	...
	fbAppVersion = "454.0.0.38.108"
	fbBuildNum   = "574142183"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS454 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS454 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS454 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios454reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
