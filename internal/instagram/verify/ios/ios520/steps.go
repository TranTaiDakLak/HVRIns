// steps.go — iOS520 verify: Spec + iOS Bloks CAA body builders + iOS headers.
//
...
const (
	...
	verifyDocID    = "375801096012207349486705547963"
	verifyBloksVer = "9f6640a4dfe5a7dbef8a547602beb113245c252866cda233a02a797683c2b120"
	verifyStylesID = "a8f4a1c2a3aae6ce1e5ac6aee64fbb75"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "520.0.0.38.101"
	fbBuildNum   = "756351453"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS520 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS520 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS520 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		Enable2FA:                nil, // iOS520 chưa hỗ trợ 2FA
		PostConfirm:              nil,
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios520reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
