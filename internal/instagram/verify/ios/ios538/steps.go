// steps.go — iOS530 verify: Spec + iOS Bloks CAA body builders + iOS headers.
//
...
const (
	...
	verifyDocID    = "37580109609847491439121543285"
	verifyBloksVer = "aee49e590aa9f864b2cd19160cfbeb043f5e0937dc49bff5eaa6c181735da00b"
	verifyStylesID = "14171f46c9ff10fd3f56cacc0cb30f61"

	// fbAppVersion / fbBuildNum — dùng cho header x-fb-request-analytics-tags (cần cố định).
	fbAppVersion = "538.0.0.59.75"
	fbBuildNum   = "821047732"
...
func verifyAccount(ctx context.Context, session *instagram.Session, cfg *instagram.VerifyConfig, outputPath string, onStatus func(uid string, msg string)) *instagram.VerifyResult {
	...
	if session.Token != "" {
		notify(fmt.Sprintf("[iOS538 Verify] User token flow — token=%.20s...", session.Token))
	} else if session.Srnonce != "" && session.SessionlessCryptedUID != "" {
		notify(fmt.Sprintf("[iOS538 Verify] Sessionless flow — srnonce=%.20s...", session.Srnonce))
	} else {
		notify("[iOS538 Verify] Không có token/srnonce — sẽ thử login lấy EAAAAU rồi verify")
	}
	...
	spec := verifybase.Spec{
		...
		Enable2FA:                nil, // iOS530 chưa hỗ trợ 2FA
		PostConfirm:              nil,
		...
		FetchToken: func(fctx context.Context, sess *instagram.Session) (string, error) {
			...
			tok, cookie, err := ios538reg.FetchIOSToken(fctx, sess.UID, sess.Password, sess.Datr, sess.Proxy, cc, notify)
			if err != nil {
				return "", err
			}
			if cookie != "" {
