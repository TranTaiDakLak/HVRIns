...
const VER_PLATFORMS_IOS: PlatformOption[] = []
...
const REG_PLATFORMS_FBLITE: PlatformOption[] = []
      ...
      <div v-show="activeStage === 'reg'" class="rp-section rp-section--reg rp-section--stage">
        ...
        <fieldset class="rp-section__disable-wrap">
          ...
          <div class="rp-subsection" :class="{ 'rp-subsection--collapsed': subCollapsed.regSettings }">
            ...
            <!-- SPC — tạo acc con từ account vừa reg ra Live (Secondary Profile Creation) -->
            <div class="rp-checks-row">
              <span class="rp-label">SPC (acc con):</span>
              <label class="rp-checkbox" title="Sau khi 1 account REG ra LIVE, dùng chính account đó làm acc gốc để tạo thêm account con qua Profile Switcher (Add account → Create new) — KHÔNG cần attestation. Acc con lưu vào SPC_Children.txt. Cooldown 35s/acc (chạy ngầm, không chậm reg). LƯU Ý: acc con ở trạng thái partially_created → check-live ra 'unknown' tới khi verify email (Phase 2 sau).">
                <input type="checkbox" v-model="form.spcEnabled" />
                <span>Tự tạo acc con khi reg ra Live</span>
              </label>
              <template v-if="form.spcEnabled">
                <span class="rp-checks-sep">·</span>
                <span class="rp-label">Số acc con/gốc:</span>
